...
"""
from . import overnight_bitsets_attacks_fixed as _bits
attacked = _bits.attacked
rebuild_metadata = _bits.rebuild_metadata
set_square = _bits.set_square
import time
import numpy as np
...
@njit(cache=False)
def attacked_scan(board, square, side):
    for d in (-17, -15):
        s = square + d * side
...
def make(board, state, move):
    ...
    old = (piece, captured, state[1], state[2], state[3], state[4], state[5], state[6])
    side = state[0]
    _bit_values_125 = side * promo if promo else piece
    set_square(board, target, _bit_values_125)
    _bit_values_126 = 0
    set_square(board, source, _bit_values_126)
    if flags & 1:
        _bit_values_128 = 0
        set_square(board, target - 16 * side, _bit_values_128)
    if flags & 2:
        rfrom = source // 16 * 16 + (7 if target > source else 0)
        rto = source + (1 if target > source else -1)
        _bit_values_132 = (board[rfrom], 0)
        set_square(board, rto, _bit_values_132[0])
        set_square(board, rfrom, _bit_values_132[1])
    rights = state[1]
    if abs(piece) == 6:
...
def unmake(board, state, move, old):
    ...
    flags = move >> 17
    side = -state[0]
    _bit_values_151 = (old[0], old[1])
    set_square(board, source, _bit_values_151[0])
    set_square(board, target, _bit_values_151[1])
    if flags & 1:
        _bit_values_153 = -side
        set_square(board, target - 16 * side, _bit_values_153)
    if flags & 2:
        rfrom = source // 16 * 16 + (7 if target > source else 0)
        rto = source + (1 if target > source else -1)
        _bit_values_157 = (board[rto], 0)
        set_square(board, rfrom, _bit_values_157[0])
        set_square(board, rto, _bit_values_157[1])
    state[0] = side
    for i in range(1, 7):
        state[i] = old[i + 1]
...
@njit(cache=False)
def has_legal_move(board, state, checked=True):
    """Terminal checks need one legal reply, not a fully validated move list."""
    side = state[0]
    if not checked:
        king = state[4 if side == 1 else 5]
        for source in range(16, 112):
            if source & 136 or board[source] != side:
                continue
            target = source + 16 * side
            if board[target] != 0:
                continue
            df = source % 16 - king % 16
            dr = source // 16 - king // 16
    ...
    if ep >= 0:
        for delta in (-1, 1):
            ...
            if source >= 0 and (not source & 136) and (board[source] == side) and (board[victim] == -side):
                _bit_values_225 = (0, 0, side)
                set_square(board, source, _bit_values_225[0])
                set_square(board, victim, _bit_values_225[1])
                set_square(board, ep, _bit_values_225[2])
                valid = not attacked(board, state[4 if side == 1 else 5], -side)
                _bit_values_227 = (side, -side, 0)
                set_square(board, source, _bit_values_227[0])
                set_square(board, victim, _bit_values_227[1])
                set_square(board, ep, _bit_values_227[2])
                if valid:
                    key ^= ZEP[ep]
                    break
    return key
...
@njit(cache=False)
def classical_phase(board, state, conversion=False):
    """Queen-aware tapered evaluation with bounded coordinated king pressure."""
    pawn_masks = (np.uint64(board[135]), np.uint64(board[133]))
    ...
    for sq in range(128):
        ...
        b += EG[p]
        if p == 1:
            if pawn_file_count(pawn_masks[c], f) > 1:
                a -= 10
                b -= 16
            if (f == 0 or pawn_file_count(pawn_masks[c], f - 1) == 0) and (f == 7 or pawn_file_count(pawn_masks[c], f + 1) == 0):
                a -= 12
                b -= 10
            passed = pawn_masks[enemy] & PASSED_PAWN_MASKS[c, sq] == 0
            if passed:
                a += r * r * 2
            ...
            if p != 6:
                ...
                a += 9 * shield
            if p == 4 and pawn_file_count(pawn_masks[c], f) == 0:
                bonus = 12 if pawn_file_count(pawn_masks[enemy], f) else 22
                a += bonus
                b += bonus // 2
        mg += side * a
        eg += side * b
    ...
    return (int(round(value * state[0])), phase)
...
@njit(cache=False)
def queen_near_king(board, state, side):
    king = state[5 if side == 1 else 4]
    for dr in range(-2, 3):
        for df in range(-2, 3):
            square = king + 16 * dr + df
            if square >= 0 and (not square & 136) and (board[square] == 5 * side):
                return True
    return False

@njit(cache=False, inline='always')
def may_give_check(board, state, move):
    if move >> 14:
        return True
    source, target = (move & 127, move >> 7 & 127)
    king = state[5 if state[0] == 1 else 4]
    df = king % 16 - source % 16
    dr = king // 16 - source // 16
    if df == 0 or dr == 0 or abs(df) == abs(dr):
        return True
    df = king % 16 - target % 16
    dr = king // 16 - target // 16
    piece = abs(board[source])
    if piece == 1:
        return abs(df) == 1 and dr == state[0]
    if piece == 2:
...
@njit(cache=False)
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, rule_weights, blend, conversion, reductions, accumulator, extensions_left, quiet_checks_left=4, quiet_attacker=0):
    control[0] += 1
    ...
    if state[3] >= 100 or repetitions >= 3 or insufficient(board):
        ...
        return 0
    if ply >= 96:
        return (-30000 + ply if checked else 0) if not has_legal_move(board, state) else evaluate_accumulator(board, state, output, rule_weights, blend, conversion, accumulator)
    if checked and depth >= 0 and (extensions_left > 0):
        depth += 1
        extensions_left -= 1
    quiescence = depth <= 0
    tt_context = context ^ np.uint64(extensions_left) * np.uint64(11400714819323198485)
    quiet_context = (quiet_attacker + 1) * 5 + quiet_checks_left + 1
    tt_context ^= np.uint64(quiet_context) * np.uint64(15485907386658061715)
    tt_context ^= np.uint64(min(state[6], 13) if blend != 0.0 else 0) * np.uint64(11562461410679940143)
    slot = int(key & np.uint64(len(ttkey) - 1))
    hint = 0
    original_alpha = alpha
    if not quiescence and ttkey[slot] == key and (ttcontext[slot] == tt_context) and (ttdata[slot, 4] == state[3]):
        hint = ttdata[slot, 3]
    ...
    if quiescence and (not checked):
        if not has_legal_move(board, state, False):
            return 0
        stand = evaluate_accumulator(board, state, output, rule_weights, blend, conversion, accumulator)
        if qdepth >= 12 or stand >= beta:
            return stand
        alpha = max(alpha, stand)
        if quiet_attacker == 0 and quiet_checks_left > 0 and (qdepth <= 4) and queen_near_king(board, state, side):
            quiet_attacker = side
    quiet_checks = quiescence and (not checked) and (quiet_attacker != 0) and (quiet_checks_left > 0)
    moves = generate(board, state, quiescence and (not checked) and (not quiet_checks))
    ...
    for index in range(len(moves)):
        ...
        if quiescence and (not checked) and quiet:
            if not quiet_checks or not may_give_check(board, state, move):
                continue
        old = make(board, state, move)
        if attacked(board, state[4 if side == 1 else 5], -side):
            unmake(board, state, move, old)
            continue
        next_quiet_checks = quiet_checks_left
        if quiescence and (not checked) and quiet:
            if not quiet_checks or not attacked(board, state[4 if state[0] == 1 else 5], side):
                unmake(board, state, move, old)
                continue
            next_quiet_checks -= 1
        legal_count += 1
        ...
        if legal_count == 1 or quiescence:
            value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, qdepth + 1 if quiescence else 0, hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, rule_weights, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker)
        else:
            value = -search(board, state, nextdepth - int(reduced), -alpha - 1, -alpha, ply + 1, np.int64(0), hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, rule_weights, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker)
            if reduced and value > alpha and (not control[1]):
                value = -search(board, state, nextdepth, -alpha - 1, -alpha, ply + 1, np.int64(0), hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, rule_weights, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker)
            if value > alpha and value < beta and (not control[1]):
                value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, np.int64(0), hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, rule_weights, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker)
        if blend != 0.0:
            update_accumulator(accumulator, weights, move, old, -1)
        unmake(board, state, move, old)
        if control[1]:
    ...
    if not quiescence:
        ...
        ttkey[slot], ttcontext[slot] = (key, tt_context)
...
@njit(cache=False)
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, rule_weights, blend, conversion, reductions):
    ...
    for index in range(len(moves)):
        ...
        value = -search(board, state, depth - 1, np.int64(-31000), -cutoff, np.int64(1), np.int64(0), hashes, hlen + 1, ctx, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, rule_weights, blend, conversion, reductions, accumulator, np.int64(2), np.int64(4), np.int64(0))
        if blend != 0.0:
            update_accumulator(accumulator, weights, move, old, -1)
        unmake(board, state, move, old)
        if control[1]:
