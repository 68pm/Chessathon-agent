...
def make(board, state, move):
    ...
    side = state[0]
    _bit_values_138 = side * promo if promo else piece
    set_square(board, target, _bit_values_138)
    _bit_values_139 = 0
    set_square(board, source, _bit_values_139)
    if flags & 1:
        _bit_values_141 = 0
        set_square(board, target - 16 * side, _bit_values_141)
    if flags & 2:
        rfrom = source // 16 * 16 + (7 if target > source else 0)
        rto = source + (1 if target > source else -1)
        _bit_values_145 = (board[rfrom], 0)
        set_square(board, rto, _bit_values_145[0])
        set_square(board, rfrom, _bit_values_145[1])
    rights = state[1]
    if abs(piece) == 6:
...
def unmake(board, state, move, old):
    ...
    flags = move >> 17
    side = -state[0]
    _bit_values_165 = (old[0], old[1])
    set_square(board, source, _bit_values_165[0])
    set_square(board, target, _bit_values_165[1])
    if flags & 1:
        _bit_values_167 = -side
        set_square(board, target - 16 * side, _bit_values_167)
    if flags & 2:
        rfrom = source // 16 * 16 + (7 if target > source else 0)
        rto = source + (1 if target > source else -1)
        _bit_values_171 = (board[rto], 0)
        set_square(board, rfrom, _bit_values_171[0])
        set_square(board, rto, _bit_values_171[1])
    state[0] = side
    for i in range(1, 6):
        state[i] = old[i + 1]
    ...
    if ep >= 0:
        for delta in (-1, 1):
            ...
            if source >= 0 and (not source & 136) and (board[source] == side) and (board[victim] == -side):
                _bit_values_246 = (0, 0, side)
                set_square(board, source, _bit_values_246[0])
                set_square(board, victim, _bit_values_246[1])
                set_square(board, ep, _bit_values_246[2])
                valid = not attacked(board, state[4 if side == 1 else 5], -side)
                _bit_values_248 = (side, -side, 0)
                set_square(board, source, _bit_values_248[0])
                set_square(board, victim, _bit_values_248[1])
                set_square(board, ep, _bit_values_248[2])
                if valid:
                    key ^= ZEP[ep]
                    break
    return key
...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, extensions_left, quiet_checks_left=4, quiet_attacker=0):
    ...
    for index in range(len(moves)):
        ...
        if legal_count == 1 or quiescence:
            ...
            value = -search(board, state, nextdepth - int(reduced), -alpha - 1, -alpha, ply + 1, 0, hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, extensions_left, next_quiet_checks, quiet_attacker)
            if reduced and value > alpha and (not control[1]):
                value = -search(board, state, nextdepth, -alpha - 1, -alpha, ply + 1, 0, hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, extensions_left, next_quiet_checks, quiet_attacker)
            if value > alpha and value < beta and (not control[1]):
                value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, 0, hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, extensions_left, next_quiet_checks, quiet_attacker)
        unmake(board, state, move, old)
        if control[1]:
            return 0
        if value > best:
...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, blend, conversion, reductions):
    ...
    for index in range(len(moves)):
        ...
        value = -search(board, state, depth - 1, -31000, -cutoff, 1, 0, hashes, hlen + 1, ctx, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, 2)
        unmake(board, state, move, old)
        if control[1]:
