...
ZSIDE = np.uint64(0xe191631a47102835)
...
def attacked(board, square, side):
    for d in (-17, -15):
        s = square + d * side
        if s >= 0 and not s & 0x88 and board[s] == side:
            return True
    for d in KNIGHT:
        s = square + d
        if s >= 0 and not s & 0x88 and board[s] == 2 * side:
            return True
    for d in KING:
        s = square + d
        if s >= 0 and not s & 0x88 and board[s] == 6 * side:
            return True
    for d in KING:
        s = square + d
        diagonal = d in (-17, -15, 15, 17)
        while s >= 0 and not s & 0x88:
            p = board[s]
            if p:
...
def generate(board, state, captures=False):
    ...
    for source in range(128):
        ...
        if piece == 1:
            ...
            for d in directions:
                target = source + d
                while target >= 0 and not target & 0x88:
                    victim = board[target]
            ...
            if piece == 6 and not captures:
                ...
                if source == home and not attacked(board, home, -side):
                    ...
                    if (rights & (1 if side == 1 else 4) and board[offset + 7] == 4 * side
                            and board[offset + 5] == 0 and board[offset + 6] == 0
                            and not attacked(board, offset + 5, -side)
                            and not attacked(board, offset + 6, -side)):
                        moves[n] = encode_move(source, offset + 6, 0, 2)
                        n += 1
                    if (rights & (2 if side == 1 else 8) and board[offset] == 4 * side
                            and board[offset + 1] == 0 and board[offset + 2] == 0 and board[offset + 3] == 0
                            and not attacked(board, offset + 3, -side)
                            and not attacked(board, offset + 2, -side)):
...
def has_legal_move(board, state, checked=True):
    ...
    if not checked:
        ...
        for source in range(16, 112):
            if source & 0x88 or board[source] != side:
                continue
            ...
            # An unaligned pawn cannot be shielding its king from a slider.
            # Promotion also keeps the destination occupied and is safe here.
...
def position_hash(board, state):
    ...
    # Only legally capturable en-passant distinguishes repetition positions.
    if ep >= 0:
        for delta in (-1, 1):
...
def classical(board, state, conversion=False):
    ...
    for sq in range(128):
        ...
        if p == 5:
            if c == 0:
                ...
                black_queen = True
    mg = 28 * (int(bishops[0] >= 2) - int(bishops[1] >= 2))
    eg = 40 * (int(bishops[0] >= 2) - int(bishops[1] >= 2))
    for sq in range(128):
        piece = board[sq]
        if sq & 0x88 or piece == 0:
            continue
        ...
        if p == 1:
            if pawns[c, f] > 1:
                a -= 10
                b -= 16
            if (f == 0 or pawns[c, f - 1] == 0) and (f == 7 or pawns[c, f + 1] == 0):
                a -= 12
                b -= 10
            passed = True
            for rr in range(rank + side, 8 if side == 1 else -1, side):
            ...
            if p != 6:
                ...
                a += 9 * shield
            if p == 4 and pawns[c, f] == 0:
                bonus = 12 if pawns[enemy, f] else 22
                a += bonus
                b += bonus // 2
        mg += side * a
        eg += side * b
    ...
    # Coordinated threats can remain dangerous with few pieces left.
    ...
    # Handwritten elementary conversion geometry, activated only against a bare king.
    # Terminal draw/mate checks in search precede this heuristic.
    if conversion and (material[0] == 0 or material[1] == 0) and max(material) >= 500:
        strong = 0 if material[0] > material[1] else 1
...
def build_accumulator(board, weights, bias):
    # Float64 accumulation bounds drift across long make/unmake sequences while
    # keeping the originally trained/exported float32 parameters unchanged.
...
@njit(cache=False)
def bishop_queen_battery(board, state):
    """A queen ray through one friendly bishop reaches the enemy king ring.

    Other occupied squares block the ray. Both colours count because a quiet
    defensive move and an attacking preparation may resolve the same threat.
    This is only a search trigger, never a fabricated mate or value bonus.
    """
    for queen in range(128):
        if queen & 0x88 or abs(board[queen]) != 5:
            continue
        side = 1 if board[queen] > 0 else -1
        king = state[5 if side == 1 else 4]
        for direction in BISHOP:
            square = queen + direction
            bishop_seen = False
            while square >= 0 and not square & 0x88:
                piece = board[square]
                if piece == 3 * side and not bishop_seen:
                    bishop_seen = True
                elif bishop_seen and max(abs(square % 16 - king % 16), abs(square // 16 - king // 16)) <= 1:
                    return True
                elif piece:
                    break
                if bishop_seen and max(abs(square % 16 - king % 16), abs(square // 16 - king // 16)) <= 1:
                    return True
                square += direction
    return False
...
@njit(cache=False)
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
           ttkey, ttcontext, ttdata, killers, history, control, deadline,
           weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, quiet_checks_left=4, quiet_attacker=0, threats_left=1):
    control[0] += 1
    ...
    if state[3] >= 100 or repetitions >= 3 or insufficient(board):
        # A checkmate takes precedence over automatic draws.
    ...
    # At most two checking nodes on a line preserve one extra ply each.
    if checked and depth >= 0 and extensions_left > 0:
        depth += 1
        extensions_left -= 1
    # One full quiet ply per line for a concrete queen/bishop battery. Unlike
    # a checking-only continuation this includes all legal defensive moves.
    if depth <= 0 and not checked and qdepth <= 4 and threats_left > 0 and bishop_queen_battery(board, state):
        depth = 1
        threats_left -= 1
    quiescence = depth <= 0
    # Future extension capacity affects the searched value, so it belongs in the TT context.
    tt_context = context ^ (np.uint64(extensions_left) * np.uint64(0x9e3779b97f4a7c15))
    # Future checking capacity and which side may use it affect searched values.
    quiet_context = (quiet_attacker + 1) * 5 + quiet_checks_left + 1
    tt_context ^= np.uint64(quiet_context) * np.uint64(0xd6e8feb86659fd93)
    tt_context ^= np.uint64(threats_left) * np.uint64(0xa0761d6478bd642f)
    ...
    if quiescence and not checked:
        # A quiet position may still be stalemate; verify one legal move before stand pat.
        ...
        # Activate once near the enemy king, then follow at most four quiet
        # checks by that attacking side. Every checked defender keeps all evasions.
    ...
    for index in range(len(moves)):
        ...
        nextdepth = depth - 1
        reduced = (reductions and not quiescence and depth >= 3 and legal_count >= 5 and quiet
                   and not checked and not attacked(board, state[4 if state[0] == 1 else 5], side))
        if legal_count == 1 or quiescence:
            value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, qdepth + 1 if quiescence else 0,
                            hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history,
                            control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker, threats_left)
        else:
            value = -search(board, state, nextdepth - int(reduced), -alpha - 1, -alpha, ply + 1, 0,
                            hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history,
                            control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker, threats_left)
            if reduced and value > alpha and not control[1]:
                value = -search(board, state, nextdepth, -alpha - 1, -alpha, ply + 1, 0,
                                hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history,
                                control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker, threats_left)
            if value > alpha and value < beta and not control[1]:
                value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, 0,
                                hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history,
                                control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker, threats_left)
        if blend != 0.0:
            update_accumulator(accumulator, weights, move, old, -1)
...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen,
    ...
    for index in range(len(moves)):
        ...
        value = -search(board, state, depth - 1, -31000, -cutoff, 1, 0,
                        ...
                        control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, 2)
        if blend != 0.0:
            update_accumulator(accumulator, weights, move, old, -1)
        unmake(board, state, move, old)
        if control[1]:
