...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
    ...
    if state[3] >= 100 or repetitions >= 3 or insufficient(board):
        ...
        return 0
    if ply >= 96:
        return (-30000 + ply if checked else 0) if not has_legal_move(board, state, checked) else evaluate_accumulator(board, state, output, blend, conversion, accumulator)
    quiescence = depth <= 0
