...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, quiet_checks_left=4, quiet_attacker=0, queen_defences_left=1):
    ...
    for index in range(len(moves)):
        ...
        if legal_count == 1 or quiescence:
            ...
            value = -search(board, state, nextdepth - int(reduced), -alpha - 1, -alpha, ply + 1, 0, hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker, queen_defences_left)
            if reduced and value > alpha and (not control[1]):
                value = -search(board, state, nextdepth, -alpha - 1, -alpha, ply + 1, 0, hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker, queen_defences_left)
            if value > alpha and value < beta and (not control[1]):
                value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, 0, hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker, queen_defences_left)
        if blend != 0.0:
            update_accumulator(accumulator, weights, move, old, -1)
        unmake(board, state, move, old)
        if control[1]:
...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, blend, conversion, reductions):
    ...
    for index in range(len(moves)):
        ...
        value = -search(board, state, depth - 1, -31000, -cutoff, 1, 0, hashes, hlen + 1, ctx, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, 2)
        if blend != 0.0:
            update_accumulator(accumulator, weights, move, old, -1)
        unmake(board, state, move, old)
        if control[1]:
