...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen,
    ...
    for index in range(len(moves)):
        ...
        # Scout later moves at their bonus-adjusted threshold. Improving
        # bounds need a full re-search; mate-band scores retain the original path.
        if index > 0 and abs(cutoff) < 28000:
            value = -search(board, state, depth - 1, -cutoff - 1, -cutoff, 1, 0,
