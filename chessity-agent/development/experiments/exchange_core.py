...
@njit(cache=False)
def exchange_reply(board, state, target, remaining):
    """Approximate optional exchange using one least-valuable legal recapture.

    Equal attackers use generator order. This is an ordering heuristic only:
    off-square tactics, optional check evasions and alternate recaptures are
    deliberately left to search. Every probe restores the caller's position.
    """
    if remaining <= 0 or not board[target] or abs(board[target]) == 6:
        return 0
    side = state[0]
    chosen, cheapest = 0, 31000
    for move in generate(board, state, True):
        if (move >> 7) & 127 != target:
            continue
        piece = abs(board[move & 127])
    ...
    if not chosen:
        return 0
    gain = MG[abs(board[target])]
    promo = (chosen >> 14) & 7
    if promo:
        gain += MG[promo] - MG[1]
    old = make(board, state, chosen)
    reply = exchange_reply(board, state, target, remaining - 1)
    unmake(board, state, chosen, old)
    return max(0, gain - reply)


@njit(cache=False)
def capture_exchange(board, state, move):
    """Material exchange estimate; retain priority for illegal/checking probes."""
    target = (move >> 7) & 127
    captured = 1 if (move >> 17) & 1 else abs(board[target])
    gain = MG[captured]
    promo = (move >> 14) & 7
    if promo:
        gain += MG[promo] - MG[1]
    ...
    value = 0 if protected else gain - exchange_reply(board, state, target, 16)
    unmake(board, state, move, old)
    return value


@njit(cache=False)
def order_moves(board, moves, hint, killers, history, ply, side, state, depth):
    ...
    for i, move in enumerate(moves):
        ...
        if move == hint:
            ...
            if depth >= 3:
                exchange = capture_exchange(board, state, move)
                if exchange < 0:
                    scores[i] = -1000000 + exchange
...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
           ...
           weights, bias, output, blend, conversion, reductions, accumulator):
    control[0] += 1
    ...
    scores = order_moves(board, moves, hint, killers, history, ply, side, state, depth)
...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen,
    ...
    for i in range(max(0, hlen - state[3] - 1), hlen):
        context += hashes[i]
    order = order_moves(board, moves, previous, killers, history, 0, state[0], state, depth)
