...
def evaluate(board, state, weights, bias, output, blend, conversion):
    ...
    if blend == 0.0 or state[1] != 0 or state[2] >= 0 or state[3] >= 70:
        return base
    hidden = bias.copy()
    for sq in range(128):
        if sq & 0x88 or board[sq] == 0:
    ...
    return base + int(round(blend * min(1500.0, max(-1500.0, residual))))
...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
    ...
    # Piece-only learned values must not override history-sensitive draw choices.
    effective_blend = 0.0 if repetitions >= 2 else blend
    if state[3] >= 100 or repetitions >= 3 or insufficient(board):
        ...
        return 0
    if ply >= 96:
        return (-30000 + ply if checked else 0) if not has_legal_move(board, state) else evaluate_accumulator(board, state, output, effective_blend, conversion, accumulator)
    ...
    if quiescence and not checked:
        ...
        if not has_legal_move(board, state, False):
            return 0
        stand = evaluate_accumulator(board, state, output, effective_blend, conversion, accumulator)
