...
@njit(cache=False)
def safe_check_opportunities(board, state, side):
    """Conservative legal direct quiet checks, counted once per attacking piece."""
    queen = False
    for sq in range(128):
        if not sq & 0x88 and board[sq] == 5 * side:
            queen = True
            break
    if not queen:
        return 0
    own_king = state[4 if side == 1 else 5]
    ...
    for source in range(128):
        ...
        for direction in directions:
            ...
            while target >= 0 and not target & 0x88 and board[target] == 0:
                df = enemy_king % 16 - target % 16
                dr = enemy_king // 16 - target // 16
                direct = False
                if piece == 2:
                    direct = abs(df) * abs(dr) == 2
                else:
                    diagonal = abs(df) == abs(dr)
                    straight = df == 0 or dr == 0
                    aligned = (diagonal and piece in (3, 5)) or (straight and piece in (4, 5))
                    if aligned:
                        step = (16 if dr > 0 else -16 if dr < 0 else 0) + (1 if df > 0 else -1 if df < 0 else 0)
                        square = target + step
                        direct = True
                        while square != enemy_king:
                            if board[square] and square != source:
                                direct = False
                                break
                            square += step
                if direct:
                    board[source], board[target] = 0, piece * side
                    safe = not attacked(board, own_king, -side) and not attacked(board, target, -side)
                    board[source], board[target] = piece * side, 0
                    if safe:
                        found = True
                        break
                if piece == 2:
            ...
            if found:
                break
        if found:
            total += 40 if piece in (2, 3) else 60 if piece == 4 else 80
...
def classical(board, state, conversion=False):
    ...
    value += safe_check_opportunities(board, state, 1) - safe_check_opportunities(board, state, -1)
...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
           ...
           weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, quiet_checks_left=4, quiet_attacker=0):
    control[0] += 1
    ...
    for index in range(len(moves)):
        ...
        if legal_count == 1 or quiescence:
            value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, qdepth + 1 if quiescence else 0,
                            ...
                            control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, extensions_left, next_quiet_checks, quiet_attacker)
        else:
