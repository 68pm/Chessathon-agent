...
def classical(board, state, conversion=False):
    ...
    weak_distance = np.full(2, 7, dtype=np.int64)
    phase = 0
    for sq in range(128):
    ...
    for sq in range(128):
        ...
        b += EG[p]
        if p == 1:
            if not white_queen and not black_queen and phase <= 8:
                defended = False
                for delta in (-1, 1):
                    defender = sq - side * 16 + delta
                    if defender >= 0 and not defender & 0x88 and board[defender] == side:
                        defended = True
                if not defended:
                    enemy_king = state[5 if side == 1 else 4]
                    distance = max(abs(enemy_king % 16 - f), abs(enemy_king // 16 - rank))
                    weak_distance[enemy] = min(weak_distance[enemy], distance)
            if pawns[c, f] > 1:
                a -= 10
                b -= 16
        ...
        mg += side * a
        eg += side * b
    # Queenless king activity towards the nearest pawn lacking pawn support.
    eg += 16 * (weak_distance[1] - weak_distance[0])
    phase = min(24, phase)
