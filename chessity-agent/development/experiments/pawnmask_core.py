...
PASSED_MASK = np.zeros((2, 128), dtype=np.uint64)
for _colour in range(2):
    for _square in range(128):
        if _square & 0x88:
            continue
        _rank, _file = divmod(_square, 16)
        _ranks = range(_rank + 1, 8) if _colour == 0 else range(_rank)
        _mask = 0
        for _rr in _ranks:
            for _ff in range(max(0, _file - 1), min(8, _file + 2)):
                _mask |= 1 << (_rr * 8 + _ff)
        PASSED_MASK[_colour, _square] = np.uint64(_mask)
...
def classical(board, state, conversion=False):
    ...
    white_pawns, black_pawns = np.uint64(0), np.uint64(0)
    phase = 0
    for sq in range(128):
    ...
    for sq in range(128):
        ...
        if p == 1:
            ...
            if (f == 0 or pawns[c, f - 1] == 0) and (f == 7 or pawns[c, f + 1] == 0):
                a -= 12
                b -= 10
            passed = (PASSED_MASK[c, sq] & (black_pawns if c == 0 else white_pawns)) == 0
            if passed:
                a += r * r * 2
                b += r * r * 5
        else:
