...
from . import overnight_root_bootstrap_core as core
...
class CompiledSearch:
    ...
    def run(self, board, seconds=0.1, soft=None, max_depth=64, max_nodes=2**60,
        ...
        previous = int(root[0])
        # A seed is not a completed full four-credit depth. Its work shares the
        ...
        for depth in range(1, max_depth + 1):
            if time.perf_counter() >= soft_deadline or control[0] >= max_nodes:
                break
            move, score, completed = core.root_iteration(
                ...
                self.weights, self.bias, self.output, self.blend, self.conversion, self.reductions, np.int64(4))
            if not completed:
