...
def null_material_ok(board):
    ...
    for square in range(128):
        ...
        kind = abs(piece)
        if 2 <= kind <= 5:
            material[0 if piece > 0 else 1] += MG[kind]
            phase += PHASE[kind]
    return phase >= 8 and material[0] >= 650 and material[1] >= 650
...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
           ...
           allow_null=True, repetition_floor=0):
    control[0] += 1
    ...
    key = hashes[hlen - 1]
    repetitions = 0
    for i in range(max(repetition_floor, hlen - state[3] - 1), hlen):
    ...
    if not quiescence and ttkey[slot] == key and ttcontext[slot] == context and ttdata[slot, 4] == state[3]:
        ...
        if ttdata[slot, 0] >= depth:
            ...
            if bound == 0 or (bound == 1 and value >= beta) or (bound == 2 and value <= alpha):
                return value
    stand = -31000
    if (allow_null and depth >= 4 and beta - alpha == 1 and not checked
            and abs(beta) < 29000 and state[3] < 60 and null_material_ok(board)
            and evaluate_accumulator(board, state, output, blend, conversion, accumulator) >= beta):
        # This virtual pass is only a bound probe, never an output move. The
        # history barrier prevents fictitious repetitions across that pass.
        old_ep = state[2]
        if len(control) >= 6:
            control[3] += 1
        state[0], state[2] = -side, -1
        nullkey = position_hash(board, state)
        saved_hash = hashes[hlen]
        hashes[hlen] = nullkey
        reduction = 2 + depth // 4
        value = -search(board, state, depth - reduction - 1, -beta, -beta + 1,
                        ply + 1, 0, hashes, hlen + 1, nullkey ^ ZSIDE,
                        ttkey, ttcontext, ttdata, killers, history, control, deadline,
                        weights, bias, output, blend, conversion, reductions, accumulator,
                        False, hlen)
        state[0], state[2], hashes[hlen] = side, old_ep, saved_hash
        if control[1]:
            return 0
        if value >= beta:
            if depth >= 7:
                if len(control) >= 6:
                    control[5] += 1
                value = search(board, state, depth - reduction, beta - 1, beta,
                               ply, qdepth, hashes, hlen, context, ttkey, ttcontext,
                               ttdata, killers, history, control, deadline,
                               weights, bias, output, blend, conversion, reductions,
                               accumulator, False, repetition_floor)
            if control[1]:
                return 0
            if value >= beta:
                if len(control) >= 6:
                    control[4] += 1
                return min(value, 28999)
