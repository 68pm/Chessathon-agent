...
@njit(cache=False)
def order_moves(board, moves, hint, killers, history, ply, side, state, exchange=False):
    ...
    for i, move in enumerate(moves):
        ...
        if move == hint:
            ...
            if exchange and captured and not promo and MG[abs(board[source])] > MG[captured]:
...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
    ...
    scores = order_moves(board, moves, hint, killers, history, ply, side, state, depth >= 3 and ply <= 1 and not checked)
...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen,
    ...
    for i in range(max(0, hlen - state[3] - 1), hlen):
        context += hashes[i]
    order = order_moves(board, moves, previous, killers, history, 0, state[0], state, depth >= 3)
