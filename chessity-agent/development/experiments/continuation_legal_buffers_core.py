...
@njit(cache=False)
def has_legal_move_buffered(board, state, storage):
    """Terminal checks need one legal reply, not a fully validated move list."""
    side = state[0]
    for move in generate_buffered(board, state, False, storage):
        old = make(board, state, move)
        valid = not attacked(board, state[4 if side == 1 else 5], -side)
        unmake(board, state, move, old)
        if valid:
...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, move_storage, score_storage):
    ...
    if state[3] >= 100 or repetitions >= 3 or insufficient(board):
        if checked and (not has_legal_move_buffered(board, state, move_storage[ply])):
            return -30000 + ply
        return 0
    if ply >= 96:
        return (-30000 + ply if checked else 0) if not has_legal_move_buffered(board, state, move_storage[ply]) else evaluate_accumulator(board, state, output, blend, conversion, accumulator)
    quiescence = depth <= 0
    ...
    if quiescence and (not checked):
        if not has_legal_move_buffered(board, state, move_storage[ply]):
            return 0
