...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
           ...
           conversion, reductions, extensions_left, quiet_checks_left=4, quiet_attacker=0):
    control[0] += 1
    ...
    for index in range(len(moves)):
        ...
        if legal_count == 1 or quiescence:
            value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, qdepth + 1 if quiescence else 0,
                            ...
                            control, deadline, conversion, reductions, extensions_left, next_quiet_checks, quiet_attacker)
        else:
...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen,
    ...
    for index in range(len(moves)):
        ...
        value = -search(board, state, depth - 1, -31000, -cutoff, 1, 0,
                        ...
                        control, deadline, conversion, reductions, 2)
        unmake(board, state, move, old)
        if control[1]:
