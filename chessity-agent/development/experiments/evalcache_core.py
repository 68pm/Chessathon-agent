...
@njit(cache=False)
def cached_evaluate(board, state, key, output, blend, conversion, accumulator, evalkeys, evalvalues):
    # Preserve any path-dependent floating-point rounding of future residual
    # networks. The selected v1.41 evaluator uses blend zero.
    if blend != 0.0:
        return evaluate_accumulator(board, state, output, blend, conversion, accumulator)
    slot = int(key & np.uint64(len(evalkeys) - 1))
    if evalkeys[slot] == key and evalvalues[slot] != -2147483648:
        return evalvalues[slot]
    value = evaluate_accumulator(board, state, output, blend, conversion, accumulator)
    evalkeys[slot], evalvalues[slot] = key, value
    return value
...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
    ...
    if state[3] >= 100 or repetitions >= 3 or insufficient(board):
        ...
        return 0
    if ply >= 96:
        return (-30000 + ply if checked else 0) if not has_legal_move(board, state) else cached_evaluate(board, state, key, output, blend, conversion, accumulator, evalkeys, evalvalues)
    quiescence = depth <= 0
    ...
    if quiescence and not checked:
        ...
        if not has_legal_move(board, state):
            return 0
        stand = cached_evaluate(board, state, key, output, blend, conversion, accumulator, evalkeys, evalvalues)
...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen,
    ...
    # Values are valid only for this root iteration's frozen evaluator/model.
    evalkeys = np.zeros(1 << 16, dtype=np.uint64)
    evalvalues = np.full(1 << 16, -2147483648, dtype=np.int64)
    context = np.uint64(0)
    for i in range(max(0, hlen - state[3] - 1), hlen):
        context += hashes[i]
