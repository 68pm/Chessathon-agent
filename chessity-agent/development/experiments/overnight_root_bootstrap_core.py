...
@njit(cache=False)
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, extensions_left, quiet_checks_left=4, quiet_attacker=0):
    control[0] += 1
    ...
    if state[3] >= 100 or repetitions >= 3 or insufficient(board):
        ...
        return 0
    if ply >= 96:
        return (-30000 + ply if checked else 0) if not has_legal_move(board, state) else classical(board, state, conversion)
    ...
    if quiescence and (not checked):
        if not has_legal_move(board, state, False):
            return 0
        stand = classical(board, state, conversion)
    ...
    for index in range(len(moves)):
        ...
        if legal_count == 1 or quiescence:
            value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, qdepth + 1 if quiescence else 0, hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, extensions_left, next_quiet_checks, quiet_attacker)
        else:
            value = -search(board, state, nextdepth - int(reduced), -alpha - 1, -alpha, ply + 1, np.int64(0), hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, extensions_left, next_quiet_checks, quiet_attacker)
            if reduced and value > alpha and (not control[1]):
                value = -search(board, state, nextdepth, -alpha - 1, -alpha, ply + 1, np.int64(0), hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, extensions_left, next_quiet_checks, quiet_attacker)
            if value > alpha and value < beta and (not control[1]):
                value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, np.int64(0), hashes, hlen + 1, childcontext, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, extensions_left, next_quiet_checks, quiet_attacker)
        unmake(board, state, move, old)
        if control[1]:
            return 0
        if value > best:
...
@njit(cache=False)
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen, ttkey, ttcontext, ttdata, killers, history, control, deadline, weights, bias, output, blend, conversion, reductions, root_quiet_checks):
    if blend != 0.0:
        raise ValueError('Classical kernel requires zero leaf-value blend')
    context = np.uint64(0)
    for i in range(max(0, hlen - state[3] - 1), hlen):
        context += hashes[i]
    ...
    for index in range(len(moves)):
        ...
        value = -search(board, state, depth - 1, np.int64(-31000), -cutoff, np.int64(1), np.int64(0), hashes, hlen + 1, ctx, ttkey, ttcontext, ttdata, killers, history, control, deadline, conversion, reductions, np.int64(2), np.int64(root_quiet_checks), np.int64(0))
        unmake(board, state, move, old)
        if control[1]:
