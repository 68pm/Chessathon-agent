...
@njit(cache=False)
def hash_after_move(parent_key, board, state, move, old):
    """Exact child key for a legal made move; preserve canonical EP via full fallback."""
    if old[3] >= 0 or state[2] >= 0:
        return position_hash(board, state)
    source, target = move & 127, (move >> 7) & 127
    piece, captured = old[0], old[1]
    key = parent_key ^ ZSIDE ^ ZCASTLE[old[2]] ^ ZCASTLE[state[1]]
    key ^= ZPIECE[piece + 6, source]
    key ^= ZPIECE[board[target] + 6, target]
    if captured:
...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
    ...
    for index in range(len(moves)):
        ...
        childkey = hash_after_move(key, board, state, move, old)
...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen,
    ...
    for index in range(len(moves)):
        ...
        if blend != 0.0:
            update_accumulator(accumulator, weights, move, old, 1)
        key = hash_after_move(hashes[hlen - 1], board, state, move, old)
        hashes[hlen] = key
