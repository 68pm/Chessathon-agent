    ...
    for sq in range(128):
        ...
        if p == 1:
            ...
            if passed:
                ...
                b += r * r * multiplier
                # In queenless play, a passed pawn supported by another pawn
                # is harder to blockade. Reward the actual diagonal support,
                # rather than treating isolated and connected passers alike.
                if not white_queen and not black_queen:
                    supported = False
                    for df in (-1, 1):
                        supporter = sq - 16 * side + df
                        if supporter >= 0 and not supporter & 0x88 and board[supporter] == side:
                            supported = True
                    if supported:
                        b += 2 * r * r
        else:
