...
OUT = ROOT / 'runs/e55-trained-rematch-staged-20260908'
...
def main():
    ...
    def verify():
        assert sha256(OUT / 'candidate.zip') == prep['zip_sha256']
    ...
    try:
        ...
        state['stage'] = 'read-only-validation'
        save_json(path, state)
        wait_for_capacity(OUT / 'watchdog-tests-capacity.json')
        child(['-X', 'utf8', '-m', 'pytest', 'tests/test_staged_package_validation.py', '-q'], OUT / 'watchdog-tests.log', timeout=60)
        wait_for_capacity(OUT / 'readonly-capacity.json')
        child(['-X', 'utf8', '-m', 'scripts.validate_package_staged', '--zip', str(OUT / 'candidate.zip'),
               '--out', str(OUT / 'read-only-check.json'), '--clock-ms', '120000', '--calls', '2'],
              OUT / 'read-only.log', timeout=240)
        check = json.loads((OUT / 'read-only-check.json').read_text())
        assert check['sha256'] == prep['zip_sha256'] and check['legal_calls'] == 2 and check['init_ms'] < 90000
