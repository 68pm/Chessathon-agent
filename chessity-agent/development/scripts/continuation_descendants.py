...
def prepare():
    ...
    timing=RUN/'legal-buffers-01/state.json'
    assert json.loads(timing.read_text())['status']=='complete'
    ...
    source=RUN/'field-review-01/own-games.json'
    games=json.loads(source.read_text())['games']
    from training.game_feedback import normalise_game
    by_round={}
    paths=[Path(__file__),ROOT/'scripts/continuation_common.py',
           ROOT/'docs/CONTINUATION_DESCENDANTS_PLAN_20260909.md',supervisor,timing,metadata,source,
           ROOT/'training/game_feedback.py',ROOT/'scripts/improvement_audit.py']
    for game in games:
        _,identity=normalise_game(game)
        path=RUN/'field-review-01/own-review/games'/identity['game_key']/'review.json'
        review=json.loads(path.read_text())
        assert review['status']=='complete'
        by_round[game['id']]=review
        paths.append(path)
    wanted=[(83,False,19),(82,True,43),(82,True,45),(81,True,8),(81,True,48),(81,True,55)]
    roots=[]
    for number,white,move in wanted:
        rows=[r for r in by_round[number]['rows'] if r['white']==white and r['fullmove']==move]
        assert len(rows)==1 and rows[0]['policy_target'] and rows[0]['policy_target']!=rows[0]['played']
        row=dict(rows[0],id=f'public-round{number}-move{move}',
                 public_submission_sha256=None,source_game=by_round[number]['identity']['game']['source_game_id'])
        restore(row)
        roots.append(row)
    save(OUT/'preparation.json',dict(candidate=str(candidate.relative_to(ROOT)),files=manifest(candidate),
        ...
        scope='Six exposed new public-game targets replayed with exact selectedv1.56; include quiet endgame labels. No fit or strength inference.'))
    print('Prepared six roots and twelve forced branches',flush=True)
