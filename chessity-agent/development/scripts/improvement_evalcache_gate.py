"""Fixed-work parity and two alternating timing passes for the exact cache."""
...
def main():
    out = ROOT / 'runs/improvement-loop-20260907/cycle-11'
    ...
    tests = out / 'cache-tests.xml'
    suite = ET.parse(tests).getroot().find('testsuite')
    assert int(suite.attrib['tests']) == 5
    ...
    shutil.copy2(ROOT / 'experiments/evalcache_core.py', prototype / 'engine/compiled_core.py')
    ...
    assert len(rows) == 11
    shutil.copy2(ROOT / 'docs/IMPROVEMENT_CYCLE_11.md', out / 'predeclaration.md')
