...
OUT = ROOT / 'runs/competition-conditional-rated-20260908'
...
def main():
    ...
    try:
        ...
        initial = run([2400, 2600], 'competition-conditional-rated-01')
        if qualifying_win(initial['games']):
            state.update(conditional_2800='triggered_by_played_2600_win', stage='2800')
            save_json(path, state)
            run([2800], 'competition-conditional-rated-2800-01')
        else:
