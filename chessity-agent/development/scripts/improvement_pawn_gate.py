"""Fixed-work parity and two alternating timing passes for exact passed-pawn masks."""
...
def main():
    out = ROOT / 'runs/improvement-loop-20260907/cycle-12'
    ...
    tests = out / 'pawn-tests.xml'
    suite = ET.parse(tests).getroot().find('testsuite')
    assert int(suite.attrib['tests']) == 7
    ...
    shutil.copy2(ROOT / 'experiments/pawnmask_core.py', prototype / 'engine/compiled_core.py')
    ...
    assert len(rows) == 11
    shutil.copy2(ROOT / 'docs/IMPROVEMENT_CYCLE_12.md', out / 'predeclaration.md')
