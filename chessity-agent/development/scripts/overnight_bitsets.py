...
OUT = ROOT / 'runs/overnight-20260909/bitsets-01'
...
def changed_source(original):
    ...
    tree.body.insert(1, ast.parse('from .overnight_bitsets_attacks import attacked, rebuild_metadata, set_square').body[0])
...
def prepare():
    ...
    parts = ROOT / 'experiments/overnight_bitsets_attacks.py'
    shutil.copy2(parts, prototype / 'engine/overnight_bitsets_attacks.py')
    ...
    experiment = ROOT / 'experiments/overnight_bitsets_core.py'
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        source_sha256={str(p.relative_to(ROOT)): digest(p) for p in [Path(__file__),
            ...
            ROOT / 'tests/test_overnight_bitsets.py', ROOT / 'docs/OVERNIGHT_BITSETS_PLAN_20260909.md',
            earlier, experiment, parts, ROOT / 'scripts/overnight_classical_kernel.py', *reviews]},
...
class WarmWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_bitsets', '--worker', label], cwd=ROOT,
