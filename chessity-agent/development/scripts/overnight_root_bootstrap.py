...
OUT = ROOT / 'runs/overnight-20260909/root-bootstrap-01'
BASE = ROOT / 'runs/overnight-20260909/coalesced-search-01/prototype'


def prepare():
    assert not OUT.exists(), 'Preserve every attempt.'
    parent = ROOT / 'runs/overnight-20260909/classical-countercheck-01'
    old = json.loads((parent / 'preparation.json').read_text(encoding='utf-8'))
    trial = json.loads((parent / 'state.json').read_text(encoding='utf-8'))
    assert trial['status'] == 'complete' and not trial['passed'] and trial['frozen_candidates']
    ...
    from scripts.root_bootstrap_transform import core_source, driver_source
    core = prototype / 'engine/compiled_core.py'
    text = core_source(core.read_text(encoding='utf-8'))
    core.write_text(text, encoding='utf-8', newline='\n')
    experiment = ROOT / 'experiments/overnight_root_bootstrap_core.py'
    assert not experiment.exists()
    experiment.write_text(text, encoding='utf-8', newline='\n')
    driver = prototype / 'engine/compiled_driver.py'
    text = driver_source(driver.read_text(encoding='utf-8'))
    driver.write_text(text, encoding='utf-8', newline='\n')
    test_driver = ROOT / 'experiments/overnight_root_bootstrap_driver.py'
    assert not test_driver.exists()
    test_driver.write_text(text.replace('from . import compiled_core as core',
        'from . import overnight_root_bootstrap_core as core'), encoding='utf-8', newline='\n')
    assert {p for p, h in manifest(prototype).items() if h != old['candidate_files']['prototype'][p]} == {'engine/compiled_core.py','engine/compiled_driver.py'}
    paths = [Path(__file__), experiment, test_driver, ROOT / 'scripts/root_bootstrap_transform.py',
        ROOT / 'tests/test_overnight_root_bootstrap.py', ROOT / 'tests/test_root_bootstrap_deadlines.py',
        ROOT / 'docs/OVERNIGHT_ROOT_BOOTSTRAP_20260909.md', parent / 'preparation.json', parent / 'state.json',
    ...
    save(OUT / 'preparation.json', dict(candidates={'baseline':str(BASE.relative_to(ROOT)),
        'prototype':str(prototype.relative_to(ROOT))}, candidate_files={'baseline':manifest(BASE),
        'prototype':manifest(prototype)}, roots=old['roots'],
        source_sha256={str(p.relative_to(ROOT)):digest(p) for p in paths},
        scope='Bounded root bootstrap and completed-child fallback. Shared actual clock/node budget; no new trained weights.'))
...
class ThreatWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_root_bootstrap', '--worker', label], cwd=ROOT,
