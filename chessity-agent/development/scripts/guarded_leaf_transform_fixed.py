...
def changed_source(pawn_source):
    ...
    search = neural_functions['search']
    slot = next(i for i, n in enumerate(search.body) if isinstance(n, ast.Assign)
                and isinstance(n.targets[0], ast.Name) and n.targets[0].id == 'slot')
    search.body[slot:slot] = ast.parse('tt_context ^= np.uint64(min(state[6], 13)) * np.uint64(0xd6e8feb86659fd93)').body
