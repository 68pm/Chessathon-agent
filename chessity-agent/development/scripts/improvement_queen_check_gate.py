"""Single bounded queen-aware/check-extension combination pilot, preserving all outcomes."""
import json
import os
import statistics
import subprocess
...
def main():
    out = ROOT / 'runs/improvement-loop-20260907/cycle-27'
    ...
    assert sha256(out / 'choice-cache.jsonl') == prep['initial_cache_sha256']
    ...
    try:
        ...
        previous_xml = ROOT / 'runs/improvement-loop-20260907/cycle-15/queen-pawn-tests.xml'
        previous_suite = ET.parse(previous_xml).getroot().find('testsuite')
        assert [int(previous_suite.attrib[k]) for k in ['tests', 'failures', 'errors', 'skipped']] == [6, 0, 0, 0]
        assert sha256(previous_xml) == prep['previous_tests_sha256']
        xml = out / 'tests.xml'

        child(['-m', 'pytest', 'tests/test_queen_check_combination.py', '-q', '--junitxml', str(xml)], out / 'tests.log')
        ...
        save_json(out / 'combined-correctness.json', dict(passed=True, retained_queen_cases=6,
            new_interaction_cases=6, previous_xml_sha256=sha256(previous_xml), new_xml_sha256=sha256(xml)))
        ...
        endgame_id = 'game-002-ply-101'
        endgame_changed = any(r['id'] == endgame_id and r['mode'] == 'clock' and r['uci'] != 'f7e8'
            for r in measurements['prototype'])
        cheap = repeats['prototype'] < repeats['baseline'] and king_changed and endgame_changed
        ...
        if cheap:
            ...
            repaired_endgame = any(r['id'] == endgame_id and max(r['regret']) <= 50
                for r in reviews['prototype']['records'][:17])
            passed = (not errors and not mates and repaired_king and repaired_endgame
            ...
            teacher = dict(clock_mean_regret=means, new_errors=errors, new_mates=mates, repaired_king=repaired_king,
                repaired_endgame=repaired_endgame, newly_requested_nodes=sum(v['new_teacher_nodes'] for v in reviews.values()))
        assert manifest(base) == prep['baseline_files'] and manifest(prototype) == prep['prototype_files']
        for name, digest in prep['source_files'].items():
            assert sha256(ROOT / name) == digest
        result = dict(status='complete', passed=passed, automatic_promotion=False, cheap_pass=cheap,
            clock_error_repeats=repeats, king_choice_changed=king_changed, endgame_choice_changed=endgame_changed, teacher_review=teacher,
