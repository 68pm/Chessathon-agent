"""Exact classical-only recursive kernel; preserve the active trained root policy."""
...
OUT = ROOT / 'runs/overnight-20260909/classical-kernel-01'
...
def changed_source(original):
    ...
    old = 'weights, bias, output, blend, conversion, reductions, accumulator'
    assert tail.count(old) >= 5
    tail = tail.replace(old, 'conversion, reductions')
    for direction in (1, -1):
        old = ('        if blend != 0.0:\n'
               f'            update_accumulator(accumulator, weights, move, old, {direction})\n')
        assert tail.count(old) == 2
        tail = tail.replace(old, '')
    old = 'evaluate_accumulator(board, state, output, blend, conversion, accumulator)'
    assert tail.count(old) == 2
    tail = tail.replace(old, 'classical(board, state, conversion)')
    old = '    accumulator = build_accumulator(board, weights, bias)\n'
    assert tail.count(old) == 1
    tail = tail.replace(old, "    if blend != 0.0:\n        raise ValueError('Classical kernel requires zero leaf-value blend')\n")
    changed = original[:offset] + tail
    parsed = ast.parse(changed)
    search = next(n for n in parsed.body if isinstance(n, ast.FunctionDef) and n.name == 'search')
    unused = {'weights', 'bias', 'output', 'blend', 'accumulator'}
    assert not (unused & {n.id for n in ast.walk(search) if isinstance(n, ast.Name)})
    assert not (unused & {n.arg for n in search.args.args})
    def unchanged(text):
...
def prepare():
    ...
    experiment = ROOT / 'experiments/overnight_classical_kernel_core.py'
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        source_sha256={str(p.relative_to(ROOT)): digest(p) for p in [Path(__file__),
            ...
            ROOT / 'tests/test_overnight_classical_kernel.py', ROOT / 'docs/OVERNIGHT_CLASSICAL_KERNEL_PLAN_20260909.md',
            earlier, experiment, *reviews]},
        fixed_nodes=250000, per_root_order='ABBA', runtime_speedup_gate=1.10, startup_reduction_gate=.20,
...
class WarmWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_classical_kernel', '--worker', label], cwd=ROOT,
...
def controller():
    ...
    try:
        ...
        useful_gain = speedup >= 1.10 or (startup_reduction >= .20 and speedup >= .98)
