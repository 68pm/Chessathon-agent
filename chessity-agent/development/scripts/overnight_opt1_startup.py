...
OUT = ROOT / 'runs/overnight-20260909/opt1-startup-01'
...
def prepare():
    ...
    paths = [Path(__file__), parent / 'preparation.json', parent / 'state.json',
...
def worker(label):
    ...
    for line in sys.stdin:
        ...
        result = agent._search.run(board, seconds, seconds, max_depth=request.get('depth', 64),
...
class ThreatWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_opt1_startup', '--worker', label], cwd=ROOT,
        ...
        for label in ('prototype', 'baseline'):
            ...
            if label == 'prototype':
                for reference in prep['parity_reference']:
                    check_stop()
                    row = workers[label].ask(dict(root=reference['root'], regime='parity', depth=reference['depth']))
