...
def archive(screen_name='root-verification-01-screen-01'):
    ...
    recovery = RUN / 'evidence-archive-recovery-01/failure.json'
    if directory.exists():
        assert recovery.exists(), 'Only resume the recorded metadata assembly failure'
        assert all(digest(ROOT / p) == h for p, h in read(recovery)['preserved_outputs'].items())
    ...
    assert {p.name for p in directory.glob('*')} == archived
    if (RUN / 'development-targets.json').exists():
        assert read(RUN / 'development-targets.json') == targets
    else:
    ...
    for item in field_state['new_games']:
        ...
        for row in negative:
            ...
            assert target is None or chess.Move.from_uci(target) in board.legal_moves
            assert row['static_value_target'] is None
            public_targets.append(dict(team_role=label, round=record['id'], url=item['url'],
