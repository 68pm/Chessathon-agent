...
from scripts.daytime_contact_allq_screen import pair_passes, qualifies_2800
...
OUT = RUN / 'contact-allq-release-audit-02'
...
def run():
    from scripts.daytime_contact_allq_screen import operationally_clean

    assert not OUT.exists()
    supervisor = json.loads((RUN/'contact-allq-screen-supervisor-02/supervisor.json').read_text())
    assert supervisor['status']=='complete' and all(r['status']=='complete' for r in supervisor['stages'])
    screen = RUN / 'contact-allq-screen-02'
    ...
    package = screen / 'contact-allq-candidate.zip'
    ...
    for path in (screen/'state.json',screen/'preparation.json',screen/'validation.json',
                 RUN/'contact-allq-02/state.json',RUN/'contact-allq-quality-02/state.json'):
        sources[str(path.relative_to(ROOT))] = digest(path)
