"""One frozen root-scout integration trial on the newly reviewed real games."""
import argparse
import json
import os
import statistics
...
OUT = ROOT / 'runs/competition-root-pvs-20260908'
...
def gate():
    ...
    passed = (len(finite) >= 13 and bool(repairs) and not new_major and not new_mate
    ...
    save_json(OUT / 'gate.json', dict(passed=passed, common_finite=len(finite), mean_regret=means,
        ...
        draw_case=rows[-1], limitation='Exposed development positions; no Elo or full-game claim.'))
...
def main():
    ...
    try:
        for stage in ['tests', 'worker', 'review']:
            ...
            args = ['-X', 'utf8', '-m', 'pytest', 'tests/test_competition_root_pvs.py', '-q'] if stage == 'tests' else [
                '-X', 'utf8', '-m', 'scripts.competition_root_pvs', '--stage', stage]
