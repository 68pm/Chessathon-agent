...
OUT = RUN/'contact-mate-01'
BASE = RUN/'fast-legal-01/prototype'

from scripts.daytime_contact_mate_transform import transform


def prepare():
    check_stop()
    assert not OUT.exists()
    parent_supervisor=RUN/'student-descendants-supervisor-02/supervisor.json'
    ...
    experiment=ROOT/'experiments/daytime_contact_mate_core.py'
    ...
    paths=[Path(__file__),ROOT/'scripts/daytime_contact_mate_transform.py',
        ROOT/'scripts/daytime_contact_mate_quality.py',ROOT/'scripts/daytime_common.py',
        ROOT/'tests/test_daytime_contact_mate.py',ROOT/'scripts/daytime_tt_hint_test_call.py',
        ROOT/'docs/DAYTIME_CONTACT_MATE_PLAN_20260909.md',experiment,parent_supervisor,
    ...
    save(OUT/'preparation.json',dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='Immediate proven quiet contact mates at qdepth zero. Exposed tactical diagnostics; playing files frozen.'))
...
class WarmWorker:
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m', 'scripts.daytime_contact_mate',
