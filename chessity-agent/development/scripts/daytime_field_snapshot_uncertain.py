...
from scripts.overnight_field_import import TEAMS, Links
from training.game_feedback import normalise_game

OUT = ROOT / 'runs/daytime-20260909/field-03'
OLD = ROOT / 'runs/overnight-20260909/field-01'


def fetch(url, path):
    check_stop()
    cached = ROOT / 'runs/daytime-20260909/field-01' / path.name
    if path.name in ('own.html', 'leader.html') and cached.exists():
...
def main():
    ...
    for label, (team_id, _) in TEAMS.items():
        ...
        parser = Links()
        parser.feed(text)
        paths = [urllib.parse.urlparse(link).path for link in parser.links]
        links = [link for link in paths if re.fullmatch(r'/game/[a-f0-9-]{36}', link)][:3]
        assert len(links) == 3
        ...
        negative = [dict(ply=r['ply'],san=r['san'],fen=r['fen'],tags=r['tags'],reward=r['reward'],
            ...
            for d in docs for r in d['rows'] if r['reward'] is not None and r['reward'] < 0]
        result['review_summary'].append(dict(label=label,already_reviewed_games=3,
            ...
            negative=sum(d['penalised'] for d in docs),
            uncertain=sum(r['reward'] is None for d in docs for r in d['rows']),largest_corrections=sorted(negative,key=lambda r:r['reward'])[:4]))
        save(OUT / 'state.json',result)
    for label in TEAMS:
