"""Single bounded pawn-proof/check-extension combination pilot, preserving all outcomes."""
import json
import os
import statistics
import subprocess
...
def main():
    out = ROOT / 'runs/improvement-loop-20260907/cycle-26'
    ...
    try:
        ...
        previous_xml = ROOT / 'runs/improvement-loop-20260907/cycle-13/pawnproof-tests.xml'
        previous_suite = ET.parse(previous_xml).getroot().find('testsuite')
        assert [int(previous_suite.attrib[k]) for k in ['tests', 'failures', 'errors', 'skipped']] == [9, 0, 0, 0]
        assert sha256(previous_xml) == prep['previous_tests_sha256']
        xml = out / 'tests.xml'

        child(['-m', 'pytest', 'tests/test_pawn_check_combination.py', '-q', '--junitxml', str(xml)], out / 'tests.log')
        ...
        save_json(out / 'combined-correctness.json', dict(passed=True, retained_pawn_cases=9,
