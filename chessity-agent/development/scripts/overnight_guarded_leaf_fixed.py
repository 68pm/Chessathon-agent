...
OUT = ROOT / 'runs/overnight-20260909/guarded-leaf-02'
...
def prepare():
    ...
    from scripts.guarded_leaf_transform_fixed import changed_source, driver_source
    ...
    experiment = ROOT / 'experiments/overnight_guarded_leaf_fixed_core.py'
    ...
    sources.update({str(p.relative_to(ROOT)): digest(p) for p in (
        Path(__file__), ROOT / 'scripts/guarded_leaf_transform_fixed.py',
        ...
        ROOT / 'tests/test_overnight_guarded_leaf_fixed.py',
        ROOT / 'docs/OVERNIGHT_GUARDED_LEAF_PLAN_20260909.md',
        ROOT / 'docs/OVERNIGHT_GUARDED_LEAF_CONTEXT_20260909.md',
        ROOT / 'tests/test_guarded_leaf_context_regression.py', experiment)})
...
class ThreatWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_guarded_leaf_fixed', '--worker', label], cwd=ROOT,
