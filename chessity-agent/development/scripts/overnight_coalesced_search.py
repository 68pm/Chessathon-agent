"""Widen root and recursive constants to coalesce compiled search variants."""
...
OUT = ROOT / 'runs/overnight-20260909/coalesced-search-01'
...
def changed_source(original):
    ...
    assert len(calls) == 1 and len(calls[0].args) == 25
    expected = {'alpha': -31000, 'ply': 1, 'qdepth': 0, 'extensions_left': 2}
    for i, name in enumerate([n.arg for n in search.args.args][:25]):
        if name in expected:
            assert ast.literal_eval(calls[0].args[i]) == expected[name]
            calls[0].args[i] = ast.parse(f'np.int64({expected[name]})', mode='eval').body
    calls[0].args += [ast.parse(f'np.int64({n})', mode='eval').body for n in (4, 0)]
    recursive = [n for n in ast.walk(search) if isinstance(n, ast.Call)
                 and isinstance(n.func, ast.Name) and n.func.id == 'search']
    changed = 0
    for call in recursive:
        if isinstance(call.args[6], ast.Constant):
            assert call.args[6].value == 0
            call.args[6] = ast.parse('np.int64(0)', mode='eval').body
            changed += 1
    assert changed == 3
...
def prepare():
    ...
    experiment = ROOT / 'experiments/overnight_coalesced_search_core.py'
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        source_sha256={str(p.relative_to(ROOT)): digest(p) for p in [Path(__file__),
            ...
            ROOT / 'tests/test_overnight_coalesced_search.py', ROOT / 'docs/OVERNIGHT_COALESCED_SEARCH_PLAN_20260909.md',
...
class WarmWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_coalesced_search', '--worker', label], cwd=ROOT,
