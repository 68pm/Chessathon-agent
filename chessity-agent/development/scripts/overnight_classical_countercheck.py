"""One bounded classical countercheck integration without neural search cost."""
...
OUT = ROOT / 'runs/overnight-20260909/classical-countercheck-01'
BASE = ROOT / 'runs/overnight-20260909/coalesced-search-01/prototype'


def prepare():
    assert not OUT.exists(), 'Preserve all prior attempts.'
    ...
    assert json.loads((ROOT / 'runs/overnight-20260909/calibrated-head-01/state.json').read_text())['passed'] is False
    OUT.mkdir()
    ...
    before = 'quiescence and (not checked) and (quiet_attacker == side) and (quiet_checks_left > 0)'
    after = 'quiescence and (not checked) and (quiet_attacker != 0) and (quiet_checks_left > 0)'
    assert text.count(before) == 1
    text = text.replace(before, after)
    core.write_text(text, encoding='utf-8', newline='\n')
    experiment = ROOT / 'experiments/overnight_classical_countercheck_core.py'
    ...
    agent.write_text(text.replace(anchor, anchor + "os.environ['NUMBA_OPT'] = '1'\n"), encoding='utf-8', newline='\n')
    assert {p for p, h in manifest(prototype).items() if h != old['candidate_files']['prototype'][p]} == {'engine/compiled_core.py', 'agent.py'}
    prior_path = ROOT / 'runs/overnight-20260909/countercheck-leaf-01/preparation.json'
    roots = json.loads(prior_path.read_text(encoding='utf-8'))['roots']
    missing = ROOT / 'runs/overnight-20260909/missing-alternatives-01/preparation.json'
    root = json.loads(missing.read_text(encoding='utf-8'))['roots'][0]
    roots.append({k:root[k] for k in ('id','start_fen','history','fen')})
    assert len(roots) == 27
    paths = [Path(__file__), experiment, ROOT / 'tests/test_overnight_classical_countercheck.py',
        ROOT / 'docs/OVERNIGHT_CLASSICAL_COUNTERCHECK_20260909.md', prior_path, missing,
        ROOT / 'scripts/overnight_hash_scout_fixed.py', ROOT / 'scripts/overnight_geometry_trial.py',
        ROOT / 'runs/overnight-20260909/calibrated-head-01/state.json', parent / 'preparation.json']
    save(OUT / 'preparation.json', dict(candidates={'baseline':str(BASE.relative_to(ROOT)),
        ...
        scope='Classical shared counterchecks with exact pawn masks and compiler OPT1. No fitted model added; exposed development diagnostics.'))
...
class ThreatWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_classical_countercheck', '--worker', label], cwd=ROOT,
