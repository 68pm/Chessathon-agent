"""Original bitset-assisted attacks with the zero-leaf-blend kernel."""
...
from scripts.overnight_classical_kernel import changed_source as specialize
from scripts.overnight_geometry_trial import BASE, ROOT, check_stop, digest, manifest, save

OUT = ROOT / 'runs/overnight-20260909/bitsets-02'




def changed_source(original):
    source = specialize(original)
    tree = ast.parse(source)
    class SquareWrites(ast.NodeTransformer):
        def __init__(self):
            self.count = 0
        def visit_Assign(self, node):
            ...
            if not all(isinstance(t, ast.Subscript) and isinstance(t.value, ast.Name)
                       and t.value.id == 'board' for t in targets):
                return node
            self.count += 1
            name = f'_bit_values_{node.lineno}'
            result = [ast.Assign(targets=[ast.Name(id=name, ctx=ast.Store())], value=node.value)]
            for i, target in enumerate(targets):
                value = (ast.Subscript(value=ast.Name(id=name, ctx=ast.Load()),
                    slice=ast.Constant(i), ctx=ast.Load()) if len(targets) > 1 else ast.Name(id=name, ctx=ast.Load()))
                result.append(ast.Expr(value=ast.Call(func=ast.Name(id='set_square', ctx=ast.Load()),
                    args=[ast.Name(id='board', ctx=ast.Load()), target.slice, value], keywords=[])))
            return result
    rewrite = SquareWrites()
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            if node.name == 'attacked':
                node.name = 'attacked_scan'
            elif node.name in ('make', 'unmake', 'position_hash'):
                rewrite.visit(node)
    assert rewrite.count == 9
    tree.body[1:1] = ast.parse('from . import overnight_bitsets_attacks_fixed as _bits\nattacked = _bits.attacked\nrebuild_metadata = _bits.rebuild_metadata\nset_square = _bits.set_square').body
...
def prepare():
    ...
    parts = ROOT / 'experiments/overnight_bitsets_attacks_fixed.py'
    shutil.copy2(parts, prototype / 'engine/overnight_bitsets_attacks_fixed.py')
    ...
    experiment = ROOT / 'experiments/overnight_bitsets_fixed_core.py'
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        source_sha256={str(p.relative_to(ROOT)): digest(p) for p in [Path(__file__),
            ...
            ROOT / 'tests/test_overnight_bitsets_fixed.py', ROOT / 'docs/OVERNIGHT_BITSETS_PLAN_20260909.md',
            earlier, experiment, parts, ROOT / 'scripts/overnight_classical_kernel.py', ROOT / 'docs/OVERNIGHT_BITSETS_CORRECTION_20260909.md', *reviews]},
...
class WarmWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_bitsets_fixed', '--worker', label], cwd=ROOT,
...
def controller():
    ...
    try:
        ...
        completed = all(r['nodes'] == 250000 or r['depth'] == 64 or abs(r['score']) > 29900 for r in fixed)
        parity = all(len({(r['score'], r['depth'], r['uci'], r['nodes']) for r in fixed if r['root'] == i}) == 1
        ...
        startup_reduction = 1 - report['workers']['prototype']['init_seconds'] / report['workers']['baseline']['init_seconds']
        useful_gain = speedup >= 1.10
        report.update(status='complete', fixed_completed=completed, fixed_score_parity=parity, startup_reduction=startup_reduction,
            ...
            passed=bool(completed and parity and useful_gain and depths['prototype'] >= depths['baseline']))
