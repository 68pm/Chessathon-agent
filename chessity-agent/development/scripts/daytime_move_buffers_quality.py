...
OUT = ROOT / 'runs/daytime-20260909/move-buffers-quality-01'
...
def prepare():
    ...
    assert json.loads((SOURCE.parent / 'move-buffers-supervisor-01/supervisor.json').read_text())['status'] == 'complete'
...
def run():
    ...
    try:
        ...
        result.update(status='complete', passed=passed, mean_regret_cp=means,
            ...
            decision='eligible_stage_one_baseline' if passed else 'retain_v155_and_diagnose')
