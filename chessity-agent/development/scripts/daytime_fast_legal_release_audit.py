"""Audit a finished, qualifying faster legal-existence candidate screen before any release mutation."""
...
from scripts.daytime_fast_legal_screen import pair_passes, qualifies_2800
...
OUT = RUN / 'fast-legal-release-audit-01'
...
def run():
    from scripts.daytime_fast_legal_screen import operationally_clean

    assert not OUT.exists()
    supervisor = json.loads((RUN/'fast-legal-screen-supervisor-01/supervisor.json').read_text())
    assert supervisor['status']=='complete' and all(r['status']=='complete' for r in supervisor['stages'])
    screen = RUN / 'fast-legal-screen-01'
    ...
    package = screen / 'fast-legal-candidate.zip'
    assert digest(package) == prep['package_sha256'] == '400c5b050aadbfc545477f77635f2bf9312ba9b94d07b252241814cee4e840d9'
    ...
    assert {'versus55','versus53','rated2400','rated2600'} <= games.keys()
    assert pair_passes(games['versus55'], 1.5) and pair_passes(games['versus53'], 1)
    ...
    for path in (screen/'state.json',screen/'preparation.json',screen/'validation.json',
                 RUN/'fast-legal-01/state.json',RUN/'fast-legal-quality-01/state.json'):
        sources[str(path.relative_to(ROOT))] = digest(path)
