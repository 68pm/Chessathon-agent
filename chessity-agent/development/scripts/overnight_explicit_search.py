"""Widen root search constants and defaults to avoid redundant compiled variants."""
...
OUT = ROOT / 'runs/overnight-20260909/explicit-search-01'
...
def prepare():
    ...
    experiment = ROOT / 'experiments/overnight_explicit_search_core.py'
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        source_sha256={str(p.relative_to(ROOT)): digest(p) for p in [Path(__file__),
            ...
            ROOT / 'tests/test_overnight_explicit_search.py', ROOT / 'docs/OVERNIGHT_EXPLICIT_SEARCH_PLAN_20260909.md',
...
class WarmWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_explicit_search', '--worker', label], cwd=ROOT,
