"""Single bounded coordinated king-pressure pilot, preserving all outcomes."""
import json
import os
import statistics
import subprocess
...
def main():
    out = ROOT / 'runs/improvement-loop-20260907/cycle-29'
    ...
    try:
        ...
        xml = out / 'tests.xml'
        child(['-m', 'pytest', 'tests/test_king_coordination.py', '-q', '--junitxml', str(xml)], out / 'tests.log')
        suite = ET.parse(xml).getroot().find('testsuite')
        assert int(suite.attrib['tests']) == 8
        assert all(int(suite.attrib[k]) == 0 for k in ['errors', 'failures', 'skipped'])
        save_json(out / 'combined-correctness.json', dict(passed=True,
            new_interaction_cases=8, new_xml_sha256=sha256(xml)))
        ...
        if cheap:
            ...
            rook_records = [r for r in reviews['prototype']['records']
                if r['id'] == 'startup19-game-003-ply-068']
            repaired_rook = len(rook_records) == 2 and all(max(r['regret']) < 200 for r in rook_records)
            passed = (not errors and not mates and repaired_king and repaired_endgame and repaired_rook
            ...
            teacher = dict(clock_mean_regret=means, new_errors=errors, new_mates=mates, repaired_king=repaired_king,
                repaired_endgame=repaired_endgame, repaired_rook=repaired_rook, newly_requested_nodes=sum(v['new_teacher_nodes'] for v in reviews.values()))
        assert manifest(base) == prep['baseline_files'] and manifest(prototype) == prep['prototype_files']
        for name, digest in prep['source_files'].items():
            assert sha256(ROOT / name) == digest
