"""Combine the legal king-step proof with exact selectedv1.56 move buffers."""
import argparse
import json
import queue
import shutil
...
def prepare():
    ...
    prior=RUN/'pawn-threat-01/preparation.json'
    roots=json.loads(prior.read_text())['roots']
    assert len(roots)==12
    ...
    experiment=ROOT/'experiments/evening_king_step_buffers_core.py'
    ...
    paths=[Path(__file__),Path(trial.__file__),ROOT/'scripts/evening_common.py',
        ROOT/'scripts/daytime_fast_legal_transform.py',experiment,prior,
        ROOT/'tests/test_evening_king_step_buffers.py',ROOT/'docs/EVENING_KING_STEP_BUFFERS_PLAN_20260909.md',
    ...
    save(OUT/'preparation.json',dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='King-step legal-existence helper combined withv1.56 buffers; no scores/order/clock changes.'))
    print('Prepared frozen king-step plus buffer trial',flush=True)
...
class Worker(trial.WarmWorker):
    def __init__(self,label):
        ...
        self.process=subprocess.Popen([sys.executable,'-X','utf8','-m','scripts.evening_king_step_buffers',
