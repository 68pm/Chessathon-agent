"""Exact piece-square lookup on v1.55 using the frozen warmed timing harness."""
import argparse
import json
import queue
import shutil
...
from scripts.daytime_piece_square_transform import transform

OUT = RUN / 'piece-square-01'
...
def prepare():
    ...
    field = json.loads((RUN/'contact-allq-quality-supervisor-02/supervisor.json').read_text())
    ...
    experiment = ROOT / 'experiments/daytime_piece_square_core.py'
    ...
    paths = [Path(__file__), Path(trial.__file__), ROOT / 'scripts/daytime_piece_square_transform.py',
        ROOT / 'scripts/daytime_pawn_extrema.py', ROOT / 'scripts/daytime_common.py', experiment,
        ROOT / 'tests/test_daytime_piece_square.py', ROOT / 'docs/DAYTIME_PIECE_SQUARE_PLAN_20260909.md',
        trace_prep, earlier]
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='Precomputed unchanged material and piece-square terms; exact evaluation and all search/rule functions preserved.'))
    print('Prepared exact piece-square tables on v1.55', flush=True)
...
class Worker(trial.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m', 'scripts.daytime_piece_square',
