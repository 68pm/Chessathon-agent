...
from scripts.pawn_mask_transform import changed_source as transform

OUT = ROOT / 'runs/overnight-20260909/pawn-masks-01'
...
def prepare():
    ...
    experiment = ROOT / 'experiments/overnight_pawn_masks_core.py'
    ...
    paths = [Path(__file__), Path(trial.__file__), ROOT / 'scripts/overnight_bitsets_fixed.py',
        ROOT / 'scripts/overnight_classical_kernel.py', ROOT / 'scripts/pawn_mask_transform.py',
        ROOT / 'scripts/overnight_combined_search.py', ROOT / 'scripts/overnight_hash_scout_fixed.py',
        ROOT / 'scripts/overnight_geometry_trial.py', ROOT / 'tests/test_overnight_pawn_masks.py',
        ROOT / 'tests/test_overnight_bitsets_fixed.py', ROOT / 'tests/test_overnight_check_prefilter.py',
        ROOT / 'docs/OVERNIGHT_PAWN_MASKS_PLAN_20260909.md', helper, experiment, earlier]
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='New exact pawn-mask evaluation mechanism on the combined runtime. Matched compiler-repaired control, not exact v1.53 ZIP. Earlier failed trials stay failed.'))
...
class WarmWorker(trial.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_pawn_masks', '--worker', label], cwd=ROOT,
