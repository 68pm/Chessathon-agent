"""One strict package check and the two already declared cycle29 games."""
import json
import sys
import zipfile
...
def main():
    out = ROOT / 'runs/improvement-loop-20260907/cycle-29-practical'
    ...
    try:
        ...
        sys.argv = ['overnight_matches', '--candidate', prep['candidate'],
            '--opponent', 'candidates/compiled-startup-plain-v1', '--stage', 'comparison',
            '--pairs', '1', '--cycle', 'cycle-29-pair',
            '--openings', 'configs/king-coordination-openings.json']
        overnight_matches.main()
        results_path = ROOT / 'runs/improvement-loop-20260907/cycle-29-pair' / ('comparison-' + candidate.name) / 'results.json'
