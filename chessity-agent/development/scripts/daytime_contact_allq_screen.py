...
QUALITY = ROOT / 'runs/daytime-20260909/contact-allq-quality-02'
OUT = ROOT / 'runs/daytime-20260909/contact-allq-screen-02'
...
def prepare():
    ...
    for directory in (SOURCE, QUALITY):
        ...
        supervisor = directory.parent / ('contact-allq-measure-supervisor-02' if directory == SOURCE else 'contact-allq-quality-supervisor-02') / 'supervisor.json'
    ...
    OUT.mkdir()
    package = OUT / 'contact-allq-candidate.zip'
    ...
    paths = [Path(__file__), ROOT / 'scripts/daytime_common.py',
        ROOT / 'tests/test_daytime_contact_allq_screen.py', ROOT / 'docs/DAYTIME_CONTACT_ALLQ_SCREEN_20260909.md',
...
def run():
    ...
    def match(row):
        ...
        cycle = 'd9-contact-allq-02-' + row['label']
