"""Per-ply move and score storage on exact v1.55 using the frozen warmed timing harness."""
import argparse
import json
import queue
import shutil
...
from scripts.daytime_move_buffers_transform import transform, transform_driver

OUT = RUN / 'move-buffers-01'
BASE = RUN / 'pawn-bitboards-01/prototype'
...
def prepare():
    ...
    assert selected['version'] == 'v1.55' and digest(ROOT.parent / 'chessity-agent.zip') == selected['sha256']
    ...
    field = json.loads((RUN/'field-supervisor-08/supervisor.json').read_text())
    assert field['status']=='complete' and all(r['status']=='complete' for r in field['stages'])
    ...
    driver_source = transform_driver((BASE/'engine/compiled_driver.py').read_text())
    (prototype/'engine/compiled_driver.py').write_text(driver_source,encoding='utf-8',newline='\n')
    experiment = ROOT / 'experiments/daytime_move_buffers_core.py'
    ...
    paths = [Path(__file__), Path(trial.__file__), ROOT / 'scripts/daytime_move_buffers_transform.py',
        ROOT / 'scripts/daytime_pawn_extrema.py', ROOT / 'scripts/daytime_common.py', experiment,
        ROOT / 'tests/test_daytime_move_buffers.py', ROOT / 'docs/DAYTIME_MOVE_BUFFERS_PLAN_20260909.md',
        trace_prep, earlier]
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='Distinct per-ply move and score storage. Identical generation/order/value/rule logic and fixed-work search required.'))
    print('Prepared per-ply move and score storage on v1.55', flush=True)
...
class Worker(trial.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m', 'scripts.daytime_move_buffers',
