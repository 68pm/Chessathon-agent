"""Avoid reversible updates for provably nonchecking quiet quiescence moves."""
...
OUT = ROOT / 'runs/overnight-20260909/check-prefilter-01'
...
def may_give_check(board, state, move):
    # Special moves may relocate a second piece or promote to another kind.
    if move >> 14:
        return True
    ...
    if df == 0 or dr == 0 or abs(df) == abs(dr):
        return True  # Conservatively retain any possible discovered check.
...
if quiescence and not checked and quiet:
    if not quiet_checks or not may_give_check(board, state, move):
        continue
""").body
    tree.body[tree.body.index(search):tree.body.index(search)] = ast.parse(CHECK_CODE).body
...
def prepare():
    ...
    experiment = ROOT / 'experiments/overnight_check_prefilter_core.py'
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        source_sha256={str(p.relative_to(ROOT)): digest(p) for p in [Path(__file__),
            ...
            ROOT / 'tests/test_overnight_check_prefilter.py', ROOT / 'docs/OVERNIGHT_CHECK_PREFILTER_PLAN_20260909.md',
            earlier, experiment, BASE.parent / 'preparation.json', *reviews]},
...
class WarmWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_check_prefilter', '--worker', label], cwd=ROOT,
