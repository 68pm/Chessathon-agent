...
OUT = ROOT / 'runs/improvement-loop-20260907/cycle-32'
...
def student():
    ...
    assert len(roots) == 2 and sum(len(r['choices']) for r in roots) == 5
    result = dict(status='initializing', pid=os.getpid(), preparation_sha256=sha256(OUT / 'preparation.json'),
        records=[], leaves=[], limits=dict(branches=5, branch_nodes=500000, branch_seconds=8,
            endpoints=21, endpoint_nodes=100000, endpoint_seconds=2))
    ...
    try:
        ...
        def evaluate(board, root_white, depth, ply, nodes, seconds, credits):
            ...
            return dict(complete=not bool(control[1]), score_root=int(score) * factor if not control[1] else None,
                nodes=int(control[0]), seconds=elapsed), context
        ...
        def features(board, root_white):
            ...
            return dict(static_root=int(core.classical(pieces, state, search.conversion)) * factor,
        ...
        for root in roots:
            ...
            for forced in root['choices']:
                ...
                if analysis['complete']:
                    assert analysis['score_root'] == root['depth6_scores'][forced]
                    ...
                    for ply in range(1, 10):
                        ...
                        tt_context = np.uint64(int(context) ^ salt)
        ...
        for leaf in leaves.values():
            ...
            save_json(path, result)
        verify(prep)
        assert len(result['records']) == 5 and len(result['leaves']) <= 21
...
def teacher():
    ...
    assert source['status'] == 'complete' and len(source['leaves']) <= 21
    ...
    try:
        for leaf in source['leaves']:
            ...
            save_json(path, report)
        assert report['newly_requested_nodes'] <= 8400000
...
def main():
    ...
    for stage in ['student', 'teacher']:
        ...
        child(['-m', 'scripts.tactical_descendant_trace', '--mode', stage], OUT / (stage + '.log'), timeout=360)
