...
from scripts.progression_common import ROOT,RUN,DEADLINE,check_stop,digest,manifest,save
...
SOURCE=RUN/'root-pvs-01'
OUT=RUN/'root-pvs-01-screen-01'
OPENINGS='configs/progression-september9-openings.json'
CYCLE_PREFIX='p9-root-pvs-01-'

def challenger_passes(pairs):
    games=[g for pair in pairs for g in pair]
    return (len(pairs)==3 and all(pair_passes(p,1) for p in pairs)
        and sum(g['score'] for g in games)>=5
        and {g['candidate_white'] for g in games if g['score']==1}=={True,False})

def all_required_pass(played):
    return (challenger_passes([played.get('versus56-c42',[]),played.get('versus56-e04',[]),played.get('versus56-b33',[])])
...
def prepare():
    ...
    paths=[Path(__file__),ROOT/'scripts/progression_common.py',ROOT/'scripts/evening_common.py',ROOT/'tests/test_progression_screen.py',
    ...
    save(OUT/'preparation.json',dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        matches=[dict(label='versus56-c42',opponent=str(BASE.relative_to(ROOT)),offset=0),
                 dict(label='versus56-e04',opponent=str(BASE.relative_to(ROOT)),offset=1),
                 dict(label='versus56-b33',opponent=str(BASE.relative_to(ROOT)),offset=2),dict(label='rated2400',level=2400,offset=3),dict(label='rated2600',level=2600,offset=3)],
        qualification='>=5/6 exactv1.56, at least1/2 in each pair and a win with each colour; >=1/2 nominal2400; >=0.5/2 nominal2600; all reviewed and operationally clean.',
...
def run():
    ...
    try:
        ...
        for row in prep['matches']:
            ...
            if row['label'].startswith('versus56-'):
                head=[g for label,pair in played.items() if label.startswith('versus56-') for g in pair]
                if score<1 or sum(g['score'] for g in head)+(6-len(head))<5:
                    state.update(status='complete',decision='reject_challenger_gate_already_impossible');return
                if len(head)==6 and not challenger_passes([played['versus56-c42'],played['versus56-e04'],played['versus56-b33']]):
        ...
        if qualifies_2800(played['rated2600']):
            played['rated2800']=match(dict(label='rated2800',level=2800,offset=3))
...
if __name__=='__main__':
    ...
    parser.add_argument('--trial',default='root-pvs-01')
    args=parser.parse_args()
    assert '/' not in args.trial and '\\' not in args.trial and '..' not in args.trial
    SOURCE=RUN/args.trial;OUT=RUN/(args.trial+'-screen-01');CYCLE_PREFIX='p9-'+args.trial+'-'
