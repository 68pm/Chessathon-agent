"""Audit a finished, qualifying buffered move-storage candidate screen before any release mutation."""
...
from scripts.daytime_move_buffers_screen import pair_passes, qualifies_2800
...
OUT = RUN / 'move-buffers-release-audit-01'
...
def run():
    from scripts.daytime_move_buffers_screen import operationally_clean

    assert not OUT.exists()
    supervisor = json.loads((RUN/'move-buffers-screen-supervisor-01/supervisor.json').read_text())
    assert supervisor['status']=='complete' and all(r['status']=='complete' for r in supervisor['stages'])
    screen = RUN / 'move-buffers-screen-01'
    ...
    package = screen / 'move-buffers-candidate.zip'
    ...
    for path in (screen/'state.json',screen/'preparation.json',screen/'validation.json',
                 RUN/'move-buffers-01/state.json',RUN/'move-buffers-quality-01/state.json'):
        sources[str(path.relative_to(ROOT))] = digest(path)
