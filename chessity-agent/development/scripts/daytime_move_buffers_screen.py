...
from scripts.daytime_common import DEADLINE, ROOT, check_stop, digest, manifest, save
...
BASE = ROOT / 'runs/daytime-20260909/pawn-bitboards-01/prototype'
SOURCE = ROOT / 'runs/daytime-20260909/move-buffers-01'
QUALITY = ROOT / 'runs/daytime-20260909/move-buffers-quality-01'
OUT = ROOT / 'runs/daytime-20260909/move-buffers-screen-01'
...
def prepare():
    ...
    for directory in (SOURCE, QUALITY):
        ...
        supervisor = directory.parent / ('move-buffers-supervisor-01' if directory == SOURCE else 'move-buffers-quality-supervisor-01') / 'supervisor.json'
    ...
    archives = {'v1.55': ROOT.parent / 'chessity-agent-v1.55.zip',
                'v1.53': ROOT.parent / 'chessity-agent-v1.53.zip'}
    archive_matches(archives['v1.55'], BASE)
    archive_matches(archives['v1.53'], OLDER)
    OUT.mkdir()
    package = OUT / 'move-buffers-candidate.zip'
    ...
    paths = [Path(__file__), ROOT / 'scripts/daytime_common.py',
        ROOT / 'tests/test_daytime_move_buffers_screen.py', ROOT / 'docs/DAYTIME_MOVE_BUFFERS_SCREEN_20260909.md',
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        matches=[dict(label='versus55', opponent=str(BASE.relative_to(ROOT)), offset=0),
        ...
        qualification='Both colours; >=1.5/2 vs exact55 and >=1/2 vs exact53, no operational failures. Both rated pairs reviewed; 2800 pair only after a clean played2600 win.',
...
def run():
    ...
    def match(row):
        ...
        cycle = 'd9-move-buffers-01-' + row['label']
    ...
    try:
        ...
        report['passed'] = (pair_passes(played['versus55'], 1.5) and pair_passes(played['versus53'], 1)
        ...
        report.update(status='complete', decision='review_for_qualified_release_and_authorised_upload'
                      if report['passed'] else 'retain_v155_and_diagnose')
    except BaseException as error:
