...
OUT = ROOT / 'runs/e55-descendants-20260908'
...
def student():
    ...
    assert len(roots) == 4 and sum(len(r['choices']) for r in roots) == 9
    result = dict(status='initializing', pid=os.getpid(), preparation_sha256=sha256(OUT / 'preparation.json'),
        records=[], leaves=[], attempts=[], limits=dict(branches=9, branch_nodes=500000, branch_seconds=8,
    ...
    try:
        ...
        for leaf in leaves.values():
            ...
            save_json(path, result)
        verify(prep)
        assert len(result['records']) == 9 and len(result['leaves']) <= 60
...
def main():
    ...
    try:
        for stage in ['student', 'teacher']:
            ...
            child(['-X', 'utf8', '-m', 'scripts.e55_descendants', '--mode', stage], OUT / (stage + '.log'), timeout=360)
