...
def run(directory):
    ...
    source_paths = [Path(__file__), ROOT / OPENINGS, ROOT / 'scripts/overnight_clock_quality.py',
        ...
        ROOT / 'scripts/feedback_matches.py', ROOT / 'training/game_feedback.py',
    ...
    report = dict(status='running', candidate=candidate, baseline=str(BASE.relative_to(ROOT)),
        ...
        scope='Serial screen only. No release selection, automatic competition upload or calibrated Elo.')
    ...
    def match(label, stage_name, pairs, offset, levels=None):
        ...
        args = ['scripts.feedback_matches', '--candidate', candidate, '--opponent', str(BASE.relative_to(ROOT)),
