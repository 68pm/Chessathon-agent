...
VARIANT = 'safe-mobility-01'
OUT = RUN / VARIANT


def prepare():
    from scripts.overnight_archive_challenge import archive_matches
    from scripts.progression_safe_mobility_transform import transform
    check_stop()
    assert VARIANT == 'safe-mobility-01' and not OUT.exists()
    ...
    experiment = ROOT / 'experiments/progression_safe_mobility_core.py'
    ...
    paths = [Path(__file__), Path(quality.__file__), Path(worker_api.__file__),
        ROOT / 'scripts/progression_common.py', ROOT / 'scripts/progression_safe_mobility_transform.py',
        experiment, previous, old, ROOT / 'tests/test_progression_safe_mobility.py',
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='Pawn-safe mobility evaluation on exact v1.56. Latest local losses and prior competition roots are development data.'))
    ...
    with (OUT / 'tests.log').open('w', encoding='utf-8') as log:
        result = subprocess.run([sys.executable, '-X', 'utf8', '-m', 'pytest', '-q',
            'tests/test_progression_safe_mobility.py', '--junitxml', str(OUT / 'tests.xml')],
    ...
    print('Prepared pawn-safe mobility candidate with correctness checks', flush=True)
...
class Worker(worker_api.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.progression_safe_mobility', '--variant', VARIANT, '--worker', label],
...
if __name__ == '__main__':
    ...
    assert VARIANT == 'safe-mobility-01'
    OUT = RUN / VARIANT
    if args.prepare:
        prepare()
    elif args.worker:
