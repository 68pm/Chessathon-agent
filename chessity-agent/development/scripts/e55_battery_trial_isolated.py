...
OUT = ROOT / 'runs/e55-battery-extension-isolated-20260908'
...
def gate():
    ...
    passed = bool(repairs) and not new_major and not new_mate and counts['prototype'] < counts['baseline'] and all(a < b for a, b in zip(means['prototype'], means['baseline'], strict=True))
    save_json(OUT / 'gate.json', dict(passed=passed, common_finite=len(finite), mean_regret=means,
        major_errors=counts, repaired_100cp=repairs, new_major=new_major, new_mate=new_mate,
...
def main():
    ...
    save_json(path, state)
    try:
        for stage in ['baseline', 'prototype', 'review']:
            stopped()
            state['stage'] = stage
            save_json(path, state)
            wait_for_capacity(OUT / (stage + '-capacity.json'))
            args = ['-X', 'utf8', '-m', 'pytest', 'tests/test_e55_battery.py', '-q'] if stage == 'tests' else [
                '-X', 'utf8', '-m', 'scripts.e55_battery_trial_isolated', '--stage', stage]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage', choices=['baseline', 'prototype', 'review'])
