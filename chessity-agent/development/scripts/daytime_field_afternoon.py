...
def run():
    ...
    try:
        ...
        for prior_path in (RUN/'field-03/state.json',RUN/'field-04/state.json',RUN/'field-06/state.json'):
            if prior_path.exists():
                ...
                known.update(g['url'] for observation in old.get('observations',[])
                    for g in observation.get('games',[]) if g['status'] in ('downloaded','already_reviewed'))
