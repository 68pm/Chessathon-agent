...
from scripts.daytime_candidate_screen import pair_passes, qualifies_2800
...
def run():
    from scripts.daytime_candidate_screen import operationally_clean

    assert not OUT.exists()
    screen = RUN / 'pawn-screen-01'
    ...
    package = screen / 'pawn-extrema-candidate.zip'
    assert digest(package) == prep['package_sha256'] == '9c5707d4dc4aa918ecb01ba284f9dd4f7469b22d50afd4b6a6d425e6f44c585e'
    ...
    assert {'versus42','versus53','rated2400','rated2600'} <= games.keys()
    assert pair_passes(games['versus42'], 1.5) and pair_passes(games['versus53'], 1)
    ...
    for path in (screen/'state.json',screen/'preparation.json',screen/'validation.json',
                 RUN/'pawn-extrema-01/state.json',RUN/'pawn-quality-01/state.json'):
        sources[str(path.relative_to(ROOT))] = digest(path)
