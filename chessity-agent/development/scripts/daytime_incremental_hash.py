...
OUT = ROOT / 'runs/daytime-20260909/incremental-hash-01'
...
def prepare():
    ...
    experimental = ROOT / 'experiments/daytime_incremental_hash_core.py'
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        source_sha256={str(p.relative_to(ROOT)): digest(p) for p in
            (Path(__file__), ROOT / 'scripts/daytime_common.py', prior, experimental,
             ROOT / 'tests/test_daytime_incremental_hash.py', ROOT / 'docs/DAYTIME_INCREMENTAL_HASH_20260909.md')},
...
class WarmWorker:
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m', 'scripts.daytime_incremental_hash',
