...
OUT = RUN / 'student-descendants-02'
...
def prepare():
    ...
    parent=RUN/'tt-hint-supervisor-02/supervisor.json'
    ...
    roots=[r for r in json.loads(public_prep.read_text())['roots'] if r['id'].startswith('own-')]
    assert len(roots)==4 and all(r['played']!=r['policy_target'] for r in roots)
    for row in roots:
        restore(row)
    paths=[Path(__file__),ROOT/'docs/DAYTIME_PUBLIC_DESCENDANTS_PLAN_20260909.md',
        ROOT/'scripts/daytime_common.py',ROOT/'scripts/daytime_public_value_diagnosis.py',
        ROOT/'training/game_feedback.py',ROOT/'training/daytime_small_value.py',
        ROOT/'scripts/improvement_audit.py',parent,probe,public_prep,metadata_path]
    save(OUT/'preparation.json',dict(candidate=str(candidate.relative_to(ROOT)),files=manifest(candidate),
        ...
        scope='Four exposed public turning points still missed by exact v1.55; independently label actual descendants before choosing a value architecture. No fit or strength test.'))
    print('Prepared four roots and eight forced branches',flush=True)
