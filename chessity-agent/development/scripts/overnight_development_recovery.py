...
OUT = ROOT / 'runs/overnight-20260909/development-recovery-01'
...
def run():
    ...
    OUT.mkdir()
    old = ROOT / 'runs/improvement-loop-20260907/overnight9-development-rematch-01-2400/rated-prototype'
    destination = ROOT / 'runs/improvement-loop-20260907/n9-dev-2400/rated-prototype'
    assert not destination.exists(), 'Never overwrite another run.'
    assert json.loads((old / 'results.json').read_text())['status'] == 'failed'
    saved_game = json.loads((old / 'game-001.json').read_text())
    from scripts.record_fastchess import audit_game
    audit_game(saved_game, dict(base_ms=120000, increment_ms=500, ply_cap=600, workers=1))
    assert saved_game['id'] == 1 and saved_game['score'] == 0 and saved_game['termination'] == 'checkmate'
    destination.mkdir(parents=True)
    preserved = {}
    for name in ('results.json', 'game-001.json'):
        shutil.copy2(old / name, destination / name)
        preserved[name] = digest(old / name)
        assert digest(destination / name) == preserved[name]
    save(OUT / 'recovery.json', dict(source=str(old.relative_to(ROOT)), destination=str(destination.relative_to(ROOT)),
        copied_sha256=preserved, first_game_replayed=False,
        reason='Completed loss saved; review failed at a 267-character Windows path. Resume its review and only the unplayed second game through the unchanged feedback entrypoint with safe path handling.'))
    source_paths = [__file__, ROOT / OPENINGS, ROOT / 'scripts/feedback_matches.py',
        ...
        ROOT / 'scripts/overnight_candidate_screen.py', ROOT / 'scripts/feedback_matches_windows.py',
        ROOT / 'docs/OVERNIGHT_DEVELOPMENT_RECOVERY_20260909.md']
    ...
    report = dict(status='running', candidate=str(CANDIDATE.relative_to(ROOT)),
        ...
        scope='Preserved-loss recovery of the predeclared bounded development diagnostic. Earlier failed clock-quality and efficiency gates remain failed. No release promotion or calibrated Elo claim. Every game reviewed; separate experimental checkpoints only.')
    ...
    try:
        for level in LEVELS:
            ...
            if not remaining_budget(datetime.now(timezone.utc)):
                report.update(status='complete', decision='stop_before_deadline')
                break
            cycle = f'n9-dev-{level}'
            args = ['scripts.feedback_matches_windows', '--candidate', str(CANDIDATE.relative_to(ROOT)),
            ...
            result = audit_feedback(feedback_path(result_path))
