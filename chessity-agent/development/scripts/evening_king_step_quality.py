"""Review completed move-buffers clock choices before any playing test."""
import argparse
import json
import statistics
...
from scripts.evening_common import ROOT, check_stop, digest, manifest, save

SOURCE = ROOT / 'runs/evening-20260909/king-step-buffers-01'
OUT = ROOT / 'runs/evening-20260909/king-step-buffers-quality-01'
    ...
    save(OUT / 'preparation.json', dict(source=str(SOURCE.relative_to(ROOT)),
        files={str(p.relative_to(ROOT)): digest(p) for p in
               (SOURCE / 'state.json', SOURCE / 'preparation.json', Path(__file__),
                ROOT / 'scripts/evening_common.py')},
...
def run():
    ...
    try:
        for index, root in enumerate(original['roots']):
            ...
            budgets = [2560000,10240000] if root['id']=='public-round83-move23' else prep['budgets']
            bests = [teacher.analyse(board, budget) for budget in budgets]
            values = {uci: [best if best['pv'][0] == uci else teacher.analyse(
                board, budget, chess.Move.from_uci(uci)) for budget, best in
                zip(budgets, bests, strict=True)] for uci in sorted({c['uci'] for c in choices})}
            reviewed = []
        ...
        result.update(status='complete', passed=passed, mean_regret_cp=means,
            ...
            decision='needs_strict_short_matches' if passed else 'retain_v156_and_diagnose')
