"""Isolated deep-exchange integration, reusing the frozen evening quality gate."""
import argparse
import json
import queue
import shutil
...
def prepare():
    ...
    experiment=ROOT/'experiments/evening_deep_exchange_core.py'
    ...
    paths=[Path(__file__),Path(trial.__file__),ROOT/'scripts/daytime_pawn_extrema.py',
        ROOT/'scripts/evening_common.py',ROOT/'scripts/evening_deep_exchange_transform.py',experiment,
        ROOT/'tests/test_evening_deep_exchange.py',ROOT/'docs/EVENING_DEEP_EXCHANGE_PLAN_20260909.md',
    ...
    save(OUT/'preparation.json',dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='Deep main-search bounded exchange ordering only on exactv1.56; no rejected search changes.'))
    print('Prepared isolated deep-exchange integration',flush=True)

class Worker(trial.Worker):
    def __init__(self,label):
        self.label=label;self.log=(OUT/f'{label}.log').open('w',encoding='utf-8');self.answers=queue.Queue()
        self.process=subprocess.Popen([sys.executable,'-X','utf8','-m','scripts.evening_deep_exchange',
