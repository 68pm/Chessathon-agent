"""Fast legal-move existence on exact v1.55 using the frozen warmed timing harness."""
import argparse
import json
import queue
import shutil
...
from scripts.daytime_fast_legal_transform import transform

OUT = RUN / 'fast-legal-01'
...
def prepare():
    ...
    field = json.loads((RUN/'field-supervisor-07/supervisor.json').read_text())
    ...
    experiment = ROOT / 'experiments/daytime_fast_legal_core.py'
    ...
    paths = [Path(__file__), Path(trial.__file__), ROOT / 'scripts/daytime_fast_legal_transform.py',
        ROOT / 'scripts/daytime_pawn_extrema.py', ROOT / 'scripts/daytime_common.py', experiment,
        ROOT / 'tests/test_daytime_fast_legal.py', ROOT / 'docs/DAYTIME_FAST_LEGAL_PLAN_20260909.md',
        trace_prep, earlier]
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='King-step existence probe with original full-list fallback. All actual search/evaluation/rule functions unchanged.'))
    print('Prepared fast legal-move existence on v1.55', flush=True)
...
class Worker(trial.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m', 'scripts.daytime_fast_legal',
