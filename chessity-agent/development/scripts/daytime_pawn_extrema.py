"""Exact pawn-extrema evaluation on v1.42, with warm ABBA timing."""
...
OUT = ROOT / 'runs/daytime-20260909/pawn-extrema-01'
...
def prepare():
    ...
    experimental = ROOT / 'experiments/daytime_pawn_extrema_core.py'
    assert not experimental.exists()
    experimental.write_text(source, encoding='utf-8', newline='\n')
    prior = ROOT / 'runs/daytime-20260909/exchange-ordering-01/preparation.json'
    roots = json.loads(prior.read_text(encoding='utf-8'))['roots']
    assert len(roots) == 16
    review_paths = []
    for label, folder in (('own', 'review'), ('leader', 'leader-review')):
        paths = sorted((ROOT / 'runs/overnight-20260909/field-01' / folder).glob('games/*/review.json'))
        assert len(paths) == 3
        review_paths.extend(paths)
        rows = [r for p in paths for r in json.loads(p.read_text())['rows']
                if r['reward'] is not None and r['reward'] < 0]
        selected, seen = [], set()
        for row in sorted(rows,key=lambda r:(r['reward'],r['fen'])):
            if row['fen'] in seen:
                continue
            seen.add(row['fen'])
            selected.append(dict(id=f'field-{label}-ply{row["ply"]}',
                **{k:row[k] for k in ('start_fen','history','fen')}))
            if len(selected) == 2:
                break
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        source_sha256={str(p.relative_to(ROOT)): digest(p) for p in
            (Path(__file__), ROOT / 'scripts/daytime_common.py', prior, experimental,
             ROOT / 'tests/test_daytime_pawn_extrema.py', ROOT / 'docs/DAYTIME_PAWN_EXTREMA_20260909.md',
             ROOT / 'scripts/daytime_pawn_extrema_transform.py', *review_paths)},
        ...
        scope='Exact pawn extrema on v1.42; no chess-score, search, model, ordering or clock changes. Development only.'))
...
class WarmWorker:
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m', 'scripts.daytime_pawn_extrema',
...
def controller():
    ...
    try:
        ...
        completed = all(r['nodes'] == 250000 or abs(r['score']) > 29900 or r['depth'] == 64 for r in fixed)
