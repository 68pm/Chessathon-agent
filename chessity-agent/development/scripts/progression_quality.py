...
def prepare():
    ...
    from scripts.progression_root_pvs_transform import transform
    check_stop()
    assert VARIANT == 'root-pvs-01' and not OUT.exists()
    ...
    experiment = ROOT / 'experiments/progression_root_pvs_core.py'
    ...
    paths = [Path(__file__), Path(quality.__file__), Path(worker_api.__file__),
        ROOT / 'scripts/progression_common.py', ROOT / 'scripts/progression_root_pvs_transform.py',
        experiment, previous, old, ROOT / 'tests/test_progression_root_pvs.py',
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='Root PVS on exact v1.56. Latest local losses and prior competition roots are development data.'))
    ...
    with (OUT / 'tests.log').open('w', encoding='utf-8') as log:
        result = subprocess.run([sys.executable, '-X', 'utf8', '-m', 'pytest', '-q',
            'tests/test_progression_root_pvs.py', '--junitxml', str(OUT / 'tests.xml')],
    ...
    print('Prepared root-PVS candidate with correctness checks', flush=True)
...
class Worker(worker_api.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.progression_quality', '--variant', VARIANT, '--worker', label],
