...
BASE = ROOT / 'runs/daytime-20260909/pawn-extrema-01/prototype'
SOURCE = ROOT / 'runs/daytime-20260909/pawn-bitboards-01'
QUALITY = ROOT / 'runs/daytime-20260909/pawn-bitboards-quality-01'
OUT = ROOT / 'runs/daytime-20260909/pawn-bitboards-screen-01'
...
def prepare():
    ...
    archives = {'v1.54': ROOT.parent / 'chessity-agent-v1.54.zip',
                'v1.53': ROOT.parent / 'chessity-agent-v1.53.zip'}
    archive_matches(archives['v1.54'], BASE)
    archive_matches(archives['v1.53'], OLDER)
    OUT.mkdir()
    package = OUT / 'pawn-bitboards-candidate.zip'
    ...
    paths = [Path(__file__), ROOT / 'scripts/daytime_common.py',
        ROOT / 'tests/test_daytime_pawn_bitboards_screen.py', ROOT / 'docs/DAYTIME_PAWN_BITBOARDS_SCREEN_20260909.md',
    ...
    save(OUT / 'preparation.json', dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        matches=[dict(label='versus54', opponent=str(BASE.relative_to(ROOT)), offset=0),
        ...
        qualification='Both colours; >=1.5/2 vs exact54 and >=1/2 vs exact53, no operational failures. Both rated pairs reviewed; 2800 pair only after a clean played2600 win.',
...
def run():
    ...
    def match(row):
        ...
        cycle = 'd9-pawn-bits-01-' + row['label']
    ...
    try:
        ...
        report['passed'] = (pair_passes(played['versus54'], 1.5) and pair_passes(played['versus53'], 1)
        ...
        report.update(status='complete', decision='review_for_qualified_release_and_authorised_upload'
                      if report['passed'] else 'retain_v154_and_diagnose')
    except BaseException as error:
