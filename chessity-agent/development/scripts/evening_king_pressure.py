"""Isolated king-pressure integration, reusing the frozen evening quality gate."""
import argparse
import json
import queue
import shutil
...
from scripts.evening_king_pressure_transform import transform

OUT=RUN/'king-pressure-01'
...
def prepare():
    ...
    experiment=ROOT/'experiments/evening_king_pressure_core.py'
    ...
    paths=[Path(__file__),Path(trial.__file__),ROOT/'scripts/daytime_pawn_extrema.py',
        ROOT/'scripts/evening_common.py',ROOT/'scripts/evening_king_pressure_transform.py',experiment,
        ROOT/'tests/test_evening_king_pressure.py',ROOT/'docs/EVENING_KING_PRESSURE_PLAN_20260909.md',
    ...
    save(OUT/'preparation.json',dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        scope='Coordinated king-pressure only on exactv1.56; no rejected search changes.'))
    print('Prepared isolated king-pressure integration',flush=True)

class Worker(trial.Worker):
    def __init__(self,label):
        self.label=label;self.log=(OUT/f'{label}.log').open('w',encoding='utf-8');self.answers=queue.Queue()
        self.process=subprocess.Popen([sys.executable,'-X','utf8','-m','scripts.evening_king_pressure',
