"""Single bounded quiet-check pilot, preserving all test and probe failures."""
import json
import os
import statistics
import subprocess
...
def main():
    out = ROOT / 'runs/improvement-loop-20260907/cycle-21-final-checks'
    ...
    try:
        ...
        xml = out / 'tests.xml'
        previous_xml = ROOT / 'runs/improvement-loop-20260907/cycle-21-recovered/tests.xml'
        previous_suite = ET.parse(previous_xml).getroot().find('testsuite')
        assert [int(previous_suite.attrib[k]) for k in ['tests', 'failures', 'errors', 'skipped']] == [10, 2, 0, 0]
        assert sha256(previous_xml) == prep['previous_tests_sha256']
        child(['-m', 'pytest', 'tests/test_quiet_checks_promotions.py', '-q', '--junitxml', str(xml)], out / 'tests.log')
        ...
        save_json(out / 'combined-correctness.json', dict(passed=True, retained_passed_cases=8,
            corrected_passed_cases=2, previous_xml_sha256=sha256(previous_xml), new_xml_sha256=sha256(xml),
            candidate_files=prep['prototype_files'], note='Only invalid promotion fixtures changed; eight passing cases were not repeated.'))
