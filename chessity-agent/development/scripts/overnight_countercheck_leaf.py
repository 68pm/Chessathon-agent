"""One bounded integration pilot for shared-budget defensive counterchecks."""
...
OUT = ROOT / 'runs/overnight-20260909/countercheck-leaf-01'
BASE = ROOT / 'runs/overnight-20260909/coalesced-search-01/prototype'


def prepare():
    from scripts.countercheck_leaf_transform import changed_source, driver_source
    assert not OUT.exists(), 'Preserve all prior attempts.'
    parent = ROOT / 'runs/overnight-20260909/guarded-leaf-02'
    ...
    experiment = ROOT / 'experiments/overnight_countercheck_leaf_core.py'
    assert not experiment.exists()
    experiment.write_text(text, encoding='utf-8', newline='\n')
    driver = prototype / 'engine/compiled_driver.py'
    driver_text = driver_source(driver.read_text(encoding='utf-8'))
    driver.write_text(driver_text, encoding='utf-8', newline='\n')
    test_driver = ROOT / 'experiments/overnight_countercheck_leaf_driver.py'
    assert not test_driver.exists()
    test_driver.write_text(driver_text.replace('from . import compiled_core as core',
        'from . import overnight_countercheck_leaf_core as core'), encoding='utf-8', newline='\n')
    sources = {str(p.relative_to(ROOT)): digest(p) for p in (
        Path(__file__), ROOT / 'scripts/countercheck_leaf_transform.py',
        ROOT / 'scripts/overnight_hash_scout_fixed.py', ROOT / 'scripts/overnight_geometry_trial.py',
        parent / 'preparation.json', ROOT / 'tests/test_overnight_countercheck_leaf.py',
        ROOT / 'docs/OVERNIGHT_COUNTERCHECK_LEAF_PLAN_20260909.md', experiment, test_driver,
        ROOT / 'runs/overnight-20260909/quiet-queen-diagnosis-02/state.json')}
    save(OUT / 'preparation.json', dict(candidates={
        ...
        roots=old['roots'], source_sha256=sources, model_sha256=digest(prototype / 'models/value.npz'),
        blend=.5, per_root_order='ABBA',
        scope='Shared four-credit counterchecks and root-scoped neural evaluation. Frozen existing parameters; exposed development roots, no strength or Elo claim.'))
...
class ThreatWorker(runner.WarmWorker):
    def __init__(self, label):
        ...
        self.process = subprocess.Popen([sys.executable, '-X', 'utf8', '-m',
            'scripts.overnight_countercheck_leaf', '--worker', label], cwd=ROOT,
