...
OUT = RUN / 'student-descendants-03'
...
def prepare():
    ...
    parent=RUN/'move-buffers-screen-supervisor-01/supervisor.json'
    assert json.loads(parent.read_text())['status']=='complete'
    audit_path=RUN/'move-buffers-release-audit-01/audit.json'
    audit=json.loads(audit_path.read_text())
    assert audit['status']=='complete' and audit['eligible']
    metadata_path=ROOT.parent/'chessity-agent-version.json'
    metadata=json.loads(metadata_path.read_text())
    assert metadata['version']=='v1.56'
    candidate=ROOT/metadata['source_version']
    archive_matches(ROOT.parent/'chessity-agent.zip',candidate)
    assert digest(ROOT.parent/'chessity-agent.zip')==metadata['sha256']==audit['sha256']
    progress_path=RUN/'move-buffers-progress-02/diagnosis.json'
    progress=json.loads(progress_path.read_text())
    assert progress['status']=='complete' and progress['completed_pairs']==4
    wanted=[('rated2600',False,15),('rated2400',False,24),
            ('rated2400',False,26),('rated2600',True,53)]
    roots=[]
    for label,white,number in wanted:
        rows=[r for r in progress['targets'] if (r['match'],r['candidate_white'],r['fullmove'])==(label,white,number)]
        assert len(rows)==1 and rows[0]['policy_target'] and rows[0]['played']!=rows[0]['policy_target']
        row=dict(rows[0],id=f'{label}-'+('white' if white else 'black')+f'-{number}')
        restore(row)
        roots.append(row)
    paths=[Path(__file__),ROOT/'docs/DAYTIME_BUFFER_DESCENDANTS_PLAN_20260909.md',
        ROOT/'scripts/daytime_common.py',ROOT/'scripts/daytime_buffer_value_diagnosis.py',
        ROOT/'training/game_feedback.py',ROOT/'training/daytime_small_value.py',
        ROOT/'scripts/improvement_audit.py',parent,audit_path,progress_path,metadata_path]
    save(OUT/'preparation.json',dict(candidate=str(candidate.relative_to(ROOT)),files=manifest(candidate),
        ...
        branch_seconds=5.0,quiescence_nodes=100000,quiescence_seconds=2.0,max_leaves=48,
        teacher_budgets=[80000,320000],max_teacher_nodes=19200000,
        scope='Four exposed v1.56 defensive/conversion targets; independently label actual descendants before choosing further value learning. No fit or strength inference.'))
    print('Prepared four v1.56 roots and eight forced branches',flush=True)
...
def student():
    ...
    try:
        ...
        for row in prep['roots']:
            for branch, forced in [('played',row['played']),('alternative',row['policy_target'])]:
                check_stop()
                board = restore(row)
        ...
        for leaf in leaves.values():
            ...
            score = core.search(pieces,position,0,-31000,31000,0,0,hashes,len(past),context,
                ...
                search.conversion,search.reductions,accumulator,search.move_storage,search.score_storage)
