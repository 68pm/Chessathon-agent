...
OUT = ROOT / 'runs/improvement-loop-20260907/cycle-37'
...
def student():
    ...
    result = dict(status='initializing', pid=os.getpid(), preparation_sha256=sha256(OUT / 'preparation.json'),
        records=[], leaves=[], attempts=[], limits=dict(branches=6, branch_nodes=500000, branch_seconds=8,
...
def main():
    ...
    for stage in ['student', 'teacher']:
        ...
        child(['-m', 'scripts.defensive_descendant_explicit', '--mode', stage], OUT / (stage + '.log'), timeout=360)
