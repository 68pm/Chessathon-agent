...
OUT = ROOT / 'runs/improvement-loop-20260907/cycle-36'
...
def student():
    ...
    assert len(roots) == 3 and sum(len(r['choices']) for r in roots) == 6
    result = dict(status='initializing', pid=os.getpid(), preparation_sha256=sha256(OUT / 'preparation.json'),
        records=[], leaves=[], limits=dict(branches=6, branch_nodes=500000, branch_seconds=8,
            endpoints=30, endpoint_nodes=100000, endpoint_seconds=2))
    ...
    try:
        ...
        for leaf in leaves.values():
            ...
            save_json(path, result)
        verify(prep)
        assert len(result['records']) == 6 and len(result['leaves']) <= 30
...
def teacher():
    ...
    assert source['status'] == 'complete' and len(source['leaves']) <= 30
    ...
    try:
        for leaf in source['leaves']:
            ...
            save_json(path, report)
        assert report['newly_requested_nodes'] <= 12000000
...
def main():
    ...
    for stage in ['student', 'teacher']:
        ...
        child(['-m', 'scripts.defensive_descendant_trace', '--mode', stage], OUT / (stage + '.log'), timeout=360)
