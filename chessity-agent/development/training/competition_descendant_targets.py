...
def classify(leaf):
    ...
    quiet = leaf.get('quiescence', {})
    quiet_score = quiet.get('score_root')
    if not quiet.get('complete') or quiet_score is None or abs(quiet_score) >= 29000:
        reasons.append('no completed finite quiescence value; not a verified static leaf')
    elif abs(quiet_score - leaf['static_root']) > 100:
        reasons.append('unresolved tactics change static value by more than 100cp; label a quieter descendant')
    ...
    return dict(id=leaf['id'], target=leaf['target'], start_fen=leaf['start_fen'],
        ...
        quiescence_root_cp=quiet_score,
