...
OUT = RUN / 'antisymmetric-value-01'
...
def prepare():
    ...
    capacity = RUN / 'antisymmetric-value-capacity-01.json'
    ...
    for offset,index in enumerate(indices):
        ...
        if group in excluded_groups or counts.get(group,0) >= 2:
            continue
        board = chess.Board(str(fens[index]))
        key = duplicate(board)
        if key in protected or not active(board):
            continue
        heldout.append(dict(fen=board.fen(), cp=float(cp[index]), split=2,
