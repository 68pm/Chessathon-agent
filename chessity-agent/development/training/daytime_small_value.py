"""One original eight-unit value fit, followed by independently labelled holdout leaves."""
import os
...
OUT = ROOT / 'runs/daytime-20260909/small-value-01'
...
def prepare():
    ...
    targets = [r for r in labels['rows'] if r['eligible'] and active(restore(r))]
    ...
    for part, limit in ((2, 16), (1, 1200), (0, 12000)):
        ...
        for index in candidates:
            ...
            if part != 0 and group in ('C09', 'D28'):
                continue
    ...
    paths = [SOURCE, LABELS, BASE / 'engine/compiled_core.py', Path(__file__),
        ROOT / 'scripts/daytime_common.py', ROOT / 'training/residual_value.py',
        ROOT / 'training/rule_value.py', ROOT / 'docs/DAYTIME_SMALL_VALUE_20260909.md']
    save(OUT / 'preparation.json', dict(seed=SEED, rows=rows, heldout_roots=heldout,
        ...
        targeted_train=len(targets), architecture='768/8 clipped ReLU residual',
        epochs=16, gradient_mix=[.8,.2], raw_runtime_cap_cp=500,
        target_group_policy='All archive C09/D28 descendants are exposed training; source split2 reserved for new fit.'))
    return classical
...
def fit(classical):
    ...
    rng = np.random.default_rng(SEED)
    parameters = [rng.normal(0,.025,(768,8)).astype(np.float32),
                  np.full(8,.25,dtype=np.float32), np.zeros(8,dtype=np.float32)]
    ...
    try:
        ...
        passed = bool(best <= .99*baseline and target_after <= .95*target_before and len(eligible) >= 8
