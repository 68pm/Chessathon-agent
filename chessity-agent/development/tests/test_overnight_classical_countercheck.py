...
from experiments import overnight_classical_countercheck_core as core
...
def test_exactly_one_search_predicate_changed():
    from pathlib import Path

    before = Path(core.__file__).with_name('overnight_pawn_masks_core.py').read_text(encoding='utf-8')
    after = Path(core.__file__).read_text(encoding='utf-8')
    anchor = 'quiescence and (not checked) and (quiet_attacker == side) and (quiet_checks_left > 0)'
    assert before.count(anchor) == 1
    assert after == before.replace(anchor, anchor.replace('quiet_attacker == side', 'quiet_attacker != 0'))
