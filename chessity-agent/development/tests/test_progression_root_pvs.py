...
import numpy as np
import pytest
from experiments import progression_root_pvs_core as core
...
def original():
    spec = importlib.util.spec_from_file_location('root_pvs_control', BASE / 'engine/compiled_core.py')
...
def test_only_root_search_changed():
    ...
    assert all(a[key] == b[key] for key in a if key != 'root_iteration')
