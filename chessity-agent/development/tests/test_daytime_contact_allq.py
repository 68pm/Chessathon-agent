...
from experiments import daytime_contact_allq_core as core
from scripts.daytime_common import RUN
from scripts.daytime_contact_allq_test_call import call
...
@pytest.mark.parametrize('qdepth',[0,1,4,11,12])
@pytest.mark.parametrize('fen',FENS)
@pytest.mark.parametrize('mirror',[False,True])
def test_actual_sacrificial_endpoints_are_mate_not_finite_training_targets(fen,mirror,qdepth):
    board=chess.Board(fen)
    if mirror:
        board=board.mirror()
    verify(board)
    score,control,_=call(core,board,seed=False,depth=0,qdepth=qdepth)
