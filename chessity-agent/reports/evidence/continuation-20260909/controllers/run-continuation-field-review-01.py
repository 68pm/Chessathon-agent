"""Serial review controller with a hard 25-minute bound and owned-child cleanup."""
import json
import os
import subprocess
import sys
...
out=RUN/'field-review-supervisor-01'
...
try:
    ...
    assert (DEADLINE-datetime.now(timezone.utc)).total_seconds()>1600
    for mode,bound in [('prepare',30),('review',1500)]:
        check_stop()
        with (out/(mode+'.log')).open('w',encoding='utf-8') as log:
            child=subprocess.Popen([sys.executable,'-X','utf8','-m','scripts.continuation_field_review',
                '--mode',mode],cwd=root,stdout=log,stderr=subprocess.STDOUT,
