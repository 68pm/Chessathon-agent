"""Bounded serial correctness and warmed measurement for legal-existence storage."""
import json
import os
import subprocess
import sys
...
out=RUN/'legal-buffers-supervisor-01'
...
try:
    ...
    assert (DEADLINE-datetime.now(timezone.utc)).total_seconds()>850
    for mode,bound,command in [('tests',90,['-m','pytest','-q','tests/test_continuation_legal_buffers.py','--junitxml='+str(RUN/'legal-buffers-01/tests.xml')]),('benchmark',650,['-m','scripts.continuation_legal_buffers'])]:
        check_stop()
