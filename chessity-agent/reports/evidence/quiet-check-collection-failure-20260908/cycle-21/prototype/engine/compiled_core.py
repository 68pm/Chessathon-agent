...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
    ...
    # At the first quiet leaf, also examine legal non-capturing checks.
    # Evasions remain unrestricted; later quiet leaves retain captures/promotions only.
    quiet_checks = quiescence and not checked and qdepth == 0
    ...
    for index in range(len(moves)):
        ...
        if attacked(board, state[4 if side == 1 else 5], -side):
            unmake(board, state, move, old)
            continue
        if quiet_checks and quiet and not attacked(board, state[4 if state[0] == 1 else 5], side):
            unmake(board, state, move, old)
            continue
        legal_count += 1
        if blend != 0.0:
