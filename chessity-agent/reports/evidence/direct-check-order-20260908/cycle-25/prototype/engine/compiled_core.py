...
@njit(cache=False)
def direct_quiet_check(board, move, side, enemy_king):
    """Moved-piece check geometry for ordering only; never alter the position."""
    source, target = move & 127, (move >> 7) & 127
    piece = abs(board[source])
    delta = enemy_king - target
    if piece == 1:
        return delta == 15 * side or delta == 17 * side
    if piece == 2:
        return delta in KNIGHT
    if piece == 6:
        return False
    df, dr = enemy_king % 16 - target % 16, enemy_king // 16 - target // 16
    diagonal = abs(df) == abs(dr) and df != 0
    straight = (df == 0) != (dr == 0)
    if not ((piece in (3, 5) and diagonal) or (piece in (4, 5) and straight)):
        return False
    step = (int(df > 0) - int(df < 0)) + 16 * (int(dr > 0) - int(dr < 0))
    square = target + step
    while square != enemy_king:
        if square != source and board[square] != 0:
            return False
        square += step
    return True
...
@njit(cache=False)
def order_moves(board, moves, hint, killers, history, ply, side, enemy_king):
    ...
    for i, move in enumerate(moves):
        ...
        if move == hint:
            ...
            if direct_quiet_check(board, move, side, enemy_king):
                scores[i] = 700000 + min(99999, scores[i])
    return scores
...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
           ...
           weights, bias, output, blend, conversion, reductions, accumulator, extensions_left):
    control[0] += 1
    ...
    scores = order_moves(board, moves, hint, killers, history, ply, side, state[5 if side == 1 else 4])
    ...
    for index in range(len(moves)):
        ...
        if legal_count == 1 or quiescence:
            value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, qdepth + 1 if quiescence else 0,
                            ...
                            control, deadline, weights, bias, output, blend, conversion, reductions, accumulator, extensions_left)
        else:
...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen,
    ...
    for i in range(max(0, hlen - state[3] - 1), hlen):
        context += hashes[i]
    order = order_moves(board, moves, previous, killers, history, 0, state[0], state[5 if state[0] == 1 else 4])
