...
def root_iteration(board, state, depth, previous, moves, bonuses, hashes, hlen,
                   ttkey, ttcontext, ttdata, killers, history, control, deadline,
                   weights, bias, output, blend, conversion, reductions, alpha, beta):
    ...
    for index in range(len(moves)):
        ...
        # Root preferences are expressed in the mover's score space. Translate
        # each bound before searching the child; mate scores never receive a bonus.
        cutoff = alpha if abs(alpha) >= 29000 else alpha - bonus
        ceiling = beta if abs(beta) >= 29000 else beta - bonus
        value = -search(board, state, depth - 1, -ceiling, -cutoff, 1, 0,
