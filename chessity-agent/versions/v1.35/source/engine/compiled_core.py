...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
    ...
    if ply >= 96:
        ...
        return (-30000 + ply if checked else 0) if len(legal) == 0 else evaluate(board, state, weights, bias, output, blend, conversion)
    quiescence = depth <= 0
    ...
    if quiescence and not checked:
        ...
        if len(legal_moves(board, state)) == 0:
            return 0
        stand = evaluate(board, state, weights, bias, output, blend, conversion)
    ...
    for index in range(len(moves)):
        ...
        if legal_count == 1 or quiescence:
            value = -search(board, state, nextdepth, -beta, -alpha, ply + 1, qdepth + 1 if quiescence else 0,
                            ...
                            control, deadline, weights, bias, output, blend, conversion, reductions)
        else:
