    ...
    def run(
        ...
        try:
            for depth in range(1, max_depth + 1):
                ...
                unstable = chosen != result.move or abs(best - result.score) >= 80
                exact_scores = sorted((v for _, v, exact in scores if exact), reverse=True)
                close = len(exact_scores) > 1 and exact_scores[0] - exact_scores[1] <= 35
                stable_iterations = 0 if unstable else stable_iterations + 1
                if adaptive_stop and depth >= 2:
                    if unstable or close:
                        soft_deadline = min(self.deadline, start + (original_soft - start) * 1.35)
                    elif stable_iterations >= 2:
                        soft_deadline = start + (original_soft - start) * 0.85
                result = Result(
                    chosen,
                    best,
                    depth,
