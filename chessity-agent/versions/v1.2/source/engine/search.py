...
class Search:
    def __init__(self, evaluator=None, style_tolerance=0):
    ...
    def negamax(self, board, depth, alpha, beta, ply):
        ...
        for move in ordered(board, moves, entry.move if entry else None, self.killers[ply], self.history):
    ...
    def run(self, board, seconds=0.1, soft=None, max_depth=64, known=None):
        ...
        style_enabled = self.style_tolerance > 0 and seconds >= 0.05 and abs(classical(board)) < 300 and phase(board) > 0.20 and not board.is_check()
        try:
            for depth in range(1, max_depth + 1):
                ...
                for move in ordered(board, moves, result.move, [], self.history):
                    self.tick()
                    value = -self.child(board, move, self.negamax, depth - 1, -MATE - 1, -alpha, 1)
                    exact = value > alpha
                    ...
                    if style_enabled and not exact and value >= alpha - self.style_tolerance:
                        value = -self.child(board, move, self.negamax, depth - 1, -MATE - 1, MATE + 1, 1)
                        exact = True
                ...
                if style_enabled and abs(best) < 29000:
                    eligible = [m for m, v, exact in scores if exact and v >= best - self.style_tolerance]
                    chosen = max(eligible, key=lambda m: style_score(board, m))
                result = Result(chosen, best, depth, self.nodes, time.monotonic() - start,
                                style_score(board, chosen))
                if abs(best) >= MATE - 100:
                    break
        except SearchTimeout:
            pass
        result.nodes, result.elapsed = self.nodes, time.monotonic() - start
        return result
