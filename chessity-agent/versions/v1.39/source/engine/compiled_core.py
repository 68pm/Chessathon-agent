...
def search(board, state, depth, alpha, beta, ply, qdepth, hashes, hlen, context,
    ...
    if state[3] >= 100 or repetitions >= 3 or insufficient(board):
        ...
        if checked and len(legal_moves(board, state)) == 0:
            return -30000 + ply
        return 0
    if ply >= 96:
        legal = legal_moves(board, state)
        return (-30000 + ply if checked else 0) if len(legal) == 0 else evaluate_accumulator(board, state, output, blend, conversion, accumulator)
    quiescence = depth <= 0
    ...
    if quiescence and not checked:
        ...
        if len(legal_moves(board, state)) == 0:
            return 0
