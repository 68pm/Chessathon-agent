...
def phase(board):
    return (
        min(
            24,
            sum(
                PHASE[k] * board.pieces_mask(k, c).bit_count()
            ...
            for sq in chess.scan_forward(pieces):
                ...
                if kind == chess.PAWN:
                    ...
                    if not pawns & adjacent:
                        ...
                        ceg -= 10
                    ahead = (
                        (chess.BB_ALL << (8 * (chess.square_rank(sq) + 1))) & chess.BB_ALL
                        if color
                        else (1 << (8 * chess.square_rank(sq))) - 1
                    )
...
def style_score(board, move):
    ...
    try:
        ...
        pressure = sum(
            (board.attacks_mask(s) & ring).bit_count()
